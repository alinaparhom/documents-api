...
function handleGenerateSummaryAction(array $env): void
{
    ...
    $model = resolveModel($env);
    $summarySystemMessage = "Ты — ассистент в режиме «Кратко ИИ».\n\n"
        . "Верни только короткий результат в 3 блоках и без лишнего текста.\n"
        . "1) Краткое содержание: 2-3 коротких пункта по сути документа.\n"
        . "2) Рекомендации: 2-3 конкретных шага, что сделать по документу.\n"
        . "3) Итог: 1 короткий вывод.\n\n"
        . "Правила: только факты из текста, без выдумок, без шапки и подписи, без длинных абзацев, максимум 7 пунктов суммарно.";
...
function handleGenerateResponseAction(array $env): void
{
    ...
    $model = resolveModel($env);
    $baseSystemMessage = "Ты — ассистент в режиме «Ответ ИИ».\n\n"
        . "Задача: нам присылают письма, мы на них отвечаем по фактам из документов.\n\n"
        . "Правила:\n"
        . "- Дай только готовый аналитический ответ по сути письма.\n"
        . "- Без пересказа документа и без разделов «Анализ/Разбор/Итог».\n"
        . "- Не выдумывай факты; используй только данные из контекста.\n"
        . "- Если данных не хватает — коротко запроси недостающие сведения.\n"
        . "- Без приветствия, подписи, реквизитов и технических комментариев.\n"
