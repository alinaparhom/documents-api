...
function handleAnalyzePaidAction(array $env): void
{
    ...
    if ($rawVisionPayload !== '') {
        ...
        if ($systemPrompt === '') {
            $systemPrompt = "Ты — аналитический ИИ-ассистент. Отвечай строго по фактам, в деловом стиле, без эмоций.\n"
                ...
                . "Пиши только основную часть по сути: без шапки, без приветствий и представлений, без подписи и без реквизитов.";
        } else {
            $systemPrompt .= "\nПиши только основную часть по сути: без шапки, без приветствий и представлений, без подписи и без реквизитов.";
        }
    ...
    $systemMessagePaid = "Ты — сотрудник строительной компании, отвечающий за официальную переписку.\n\n"
        ...
        . "Правила:\n"
        . "- Не добавляй шапку (кому, от кого), приветствия/представления, подпись и реквизиты.\n"
        ...
        . "- ВЕРНИ ТОЛЬКО ОСНОВНОЙ ТЕКСТ ПО СУТИ, БЕЗ ВСТУПЛЕНИЙ И ЗАКЛЮЧИТЕЛЬНЫХ ФРАЗ.\n\n";
...
function handleGenerateResponseAction(array $env): void
{
    ...
    $baseSystemMessage = "Ты — сотрудник строительной компании, отвечающий за официальную переписку.\n\n"
        ...
        . "Правила:\n"
        . "- Не добавляй шапку письма, приветствия/представления, подпись, должность и служебные реквизиты.\n"
        ...
        . "- ВЕРНИ ТОЛЬКО ГОТОВЫЙ ТЕКСТ ОТВЕТА ПО СУТИ, БЕЗ АНАЛИЗА И ПОЯСНЕНИЙ.\n"
        . "- БЕЗ ВСТУПЛЕНИЯ, БЕЗ ФРАЗ-ПРЕДСТАВЛЕНИЙ И БЕЗ ЗАКЛЮЧИТЕЛЬНОЙ ПОДПИСИ.\n";
