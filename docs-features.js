export function createDocumentsFeatures(core) {
  ...
  var OCR_SNAPSHOT_DELIVERY_VERSION = 2;
  var OCR_BACKFILL_POLL_INTERVAL_MS = 7000;
  var OCR_ADMIN_PAGE_SIZE = 50;
...
function ensureResponsesStyle() {
    ...
    style.textContent = '' +
      ...
      '.documents-private-ocr-modal{z-index:2200;padding:0;align-items:stretch}.documents-private-ocr-panel{width:100%;height:100vh;height:100dvh;max-height:none;border-radius:0;background:#f8fafc}.documents-private-ocr-panel>.documents-brief-header{flex:0 0 auto;background:#fff}' +
      '.documents-private-ocr-tabs{display:flex;flex:0 0 auto;gap:4px;padding:8px 14px 0;background:#fff;border-bottom:1px solid #e2e8f0}.documents-private-ocr-tab{min-height:40px;padding:0 16px;border:0;border-bottom:3px solid transparent;background:transparent;color:#64748b;font-size:12px;font-weight:850;cursor:pointer}.documents-private-ocr-tab:hover{color:#1d4ed8;background:#f8fafc}.documents-private-ocr-tab.is-active{border-bottom-color:#2563eb;color:#1d4ed8}.documents-private-ocr-tab:focus-visible{outline:3px solid rgba(59,130,246,.2);outline-offset:-3px}.documents-private-ocr-backfill[hidden],.documents-private-telegram-audit[hidden],.documents-private-ocr-monitor[hidden]{display:none}' +
      '.documents-private-ocr-backfill{flex:0 0 auto;margin:12px 14px 0;padding:14px 16px;border:1px solid #bfdbfe;border-radius:14px;background:#fff;box-shadow:0 8px 24px rgba(15,23,42,.05)}.documents-private-ocr-backfill__top{display:flex;align-items:center;justify-content:space-between;gap:16px}.documents-private-ocr-backfill__copy{min-width:0;max-width:920px}' +
      '.documents-private-ocr-backfill__title{font-size:14px;font-weight:900;color:#0f172a}.documents-private-ocr-backfill__summary{margin-top:4px;font-size:12px;line-height:1.45;color:#475569;overflow-wrap:anywhere}.documents-private-ocr-backfill__actions{display:flex;align-items:center;gap:8px;flex:0 0 auto}' +
      '.documents-private-ocr-backfill__button{min-height:36px;padding:0 15px;border:1px solid #2563eb;border-radius:9px;background:#2563eb;color:#fff;font-size:12px;font-weight:850;cursor:pointer;box-shadow:0 5px 12px rgba(37,99,235,.16);transition:background .16s ease,border-color .16s ease,transform .16s ease}.documents-private-ocr-backfill__button--secondary{border-color:#cbd5e1;background:#fff;color:#334155;box-shadow:none}.documents-private-ocr-backfill__button:hover:not(:disabled){background:#1d4ed8;border-color:#1d4ed8;transform:translateY(-1px)}.documents-private-ocr-backfill__button--secondary:hover:not(:disabled){background:#f1f5f9;border-color:#94a3b8;color:#0f172a}.documents-private-ocr-backfill__button:focus-visible{outline:3px solid rgba(59,130,246,.22);outline-offset:2px}.documents-private-ocr-backfill__button:disabled{cursor:wait;opacity:.6}' +
      '.documents-private-ocr-backfill__track{height:7px;margin-top:11px;border-radius:999px;background:#dbeafe;overflow:hidden}.documents-private-ocr-backfill__progress{height:100%;width:0;border-radius:inherit;background:linear-gradient(90deg,#2563eb,#0ea5e9);transition:width .25s ease}.documents-private-ocr-backfill__stats{display:flex;flex-wrap:wrap;gap:5px 12px;margin-top:9px;color:#64748b;font-size:11px;line-height:1.35}.documents-private-ocr-backfill__cron{margin-top:7px;color:#64748b;font-size:11px;line-height:1.4}.documents-private-ocr-backfill__cron.is-active{color:#047857}.documents-private-ocr-backfill__cron.is-error{color:#b91c1c}' +
      '.documents-private-ocr-job{margin-top:11px;padding:10px 11px;border:1px solid rgba(59,130,246,.2);border-radius:12px;background:rgba(255,255,255,.82)}.documents-private-ocr-job[hidden]{display:none}.documents-private-ocr-job__head{display:flex;align-items:center;justify-content:space-between;gap:12px}.documents-private-ocr-job__label{min-width:0;color:#1e3a8a;font-size:11px;font-weight:800;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.documents-private-ocr-job__percent{flex:0 0 auto;color:#2563eb;font-size:12px;font-weight:900;font-variant-numeric:tabular-nums}.documents-private-ocr-job__track{height:6px;margin-top:8px;border-radius:999px;background:#dbeafe;overflow:hidden}.documents-private-ocr-job__progress{height:100%;width:0;border-radius:inherit;background:linear-gradient(90deg,#2563eb,#22c55e);transition:width .3s ease}.documents-private-ocr-job__meta{margin-top:7px;color:#64748b;font-size:11px;line-height:1.35}' +
      '.documents-private-ocr-backfill__failure{margin-top:8px;padding:8px 10px;border-radius:10px;background:#fff1f2;color:#9f1239;font-size:11px;line-height:1.4;overflow-wrap:anywhere}.documents-private-ocr-backfill__failure[hidden]{display:none}' +
      '.documents-private-telegram-audit{display:flex;flex:1 1 auto;flex-direction:column;min-height:0;margin:12px 14px 14px;border:1px solid #dbe3ee;border-radius:14px;background:#fff;box-shadow:0 6px 20px rgba(15,23,42,.04);overflow:hidden}.documents-private-telegram-audit__top{display:flex;align-items:center;justify-content:space-between;gap:14px;padding:14px}.documents-private-telegram-audit__copy{min-width:0}.documents-private-telegram-audit__title{font-size:15px;font-weight:900;color:#0f172a}.documents-private-telegram-audit__summary{margin-top:3px;color:#475569;font-size:12px;line-height:1.4}.documents-private-telegram-audit__actions{display:flex;align-items:center;gap:7px;flex:0 0 auto}.documents-private-telegram-audit__track{height:5px;background:#e2e8f0;overflow:hidden}.documents-private-telegram-audit__progress{height:100%;width:0;background:linear-gradient(90deg,#0ea5e9,#10b981);transition:width .25s ease}.documents-private-telegram-audit__stats{display:flex;flex-wrap:wrap;gap:5px 12px;padding:8px 14px;color:#64748b;font-size:11px;line-height:1.35;border-bottom:1px solid #e2e8f0}.documents-private-telegram-audit__warning{padding:8px 14px;background:#fff7ed;color:#9a3412;font-size:11px;line-height:1.4}.documents-private-telegram-audit__warning[hidden]{display:none}.documents-private-telegram-audit__toolbar{display:flex;flex:0 0 auto;align-items:center;gap:9px;padding:10px 14px;border-bottom:1px solid #e2e8f0;background:#f8fafc}.documents-private-telegram-audit__select{box-sizing:border-box;height:38px;padding:0 32px 0 11px;border:1px solid #cbd5e1;border-radius:9px;background:#fff;color:#334155;font:inherit;font-size:12px;outline:none}.documents-private-telegram-audit__select:focus{border-color:#60a5fa;box-shadow:0 0 0 3px rgba(37,99,235,.1)}.documents-private-telegram-audit__table-wrap{flex:1 1 auto;min-height:0;overflow-x:auto;overflow-y:auto;scrollbar-gutter:stable both-edges}.documents-private-telegram-audit__table{width:100%;min-width:820px;border-collapse:collapse;color:#334155;font-size:11px}.documents-private-telegram-audit__table th,.documents-private-telegram-audit__table td{padding:9px 11px;border-bottom:1px solid #eef2f7;text-align:left;vertical-align:top}.documents-private-telegram-audit__table th{position:sticky;top:0;z-index:1;background:#f8fafc;color:#475569;font-weight:850}.documents-private-telegram-audit__table tbody tr:last-child td{border-bottom:0}.documents-private-telegram-audit__user{color:#0f172a;font-weight:800}.documents-private-telegram-audit__name{margin-top:2px;color:#64748b;font-weight:500}.documents-private-telegram-audit__status{display:inline-flex;padding:3px 6px;border-radius:999px;background:#f1f5f9;color:#475569;font-weight:800;white-space:nowrap}.documents-private-telegram-audit__status--valid{background:#ecfdf5;color:#047857}.documents-private-telegram-audit__status--reconciled{background:#eff6ff;color:#1d4ed8}.documents-private-telegram-audit__status--failed{background:#fff1f2;color:#be123c}.documents-private-telegram-audit__error{max-width:280px;margin-top:3px;color:#be123c;overflow-wrap:anywhere}' +
      '.documents-private-ocr-monitor{display:flex;flex:1 1 auto;flex-direction:column;gap:10px;padding:12px 14px 14px;min-height:0;overflow:hidden}.documents-private-ocr-summary{display:grid;flex:0 0 auto;grid-template-columns:repeat(5,minmax(0,1fr));gap:8px}.documents-private-ocr-summary__card{min-width:0;padding:10px 12px;border:1px solid #e2e8f0;border-radius:11px;background:#fff;box-shadow:0 3px 10px rgba(15,23,42,.035)}.documents-private-ocr-summary__value{font-size:21px;font-weight:900;line-height:1;color:#0f172a;font-variant-numeric:tabular-nums}.documents-private-ocr-summary__label{margin-top:5px;color:#64748b;font-size:11px;line-height:1.3}.documents-private-ocr-summary__card--attention{border-color:#fecaca;background:#fff7f7}.documents-private-ocr-summary__card--attention .documents-private-ocr-summary__value{color:#b91c1c}.documents-private-ocr-summary__card--success{border-color:#bbf7d0;background:#f0fdf4}.documents-private-ocr-summary__card--success .documents-private-ocr-summary__value{color:#047857}' +
      '.documents-private-ocr-toolbar{display:flex;flex:0 0 auto;align-items:center;gap:10px;padding:9px;border:1px solid #e2e8f0;border-radius:11px;background:#fff;box-shadow:0 4px 14px rgba(15,23,42,.04)}.documents-private-ocr-search{box-sizing:border-box;flex:1 1 320px;min-width:200px;height:38px;padding:0 13px;border:1px solid #cbd5e1;border-radius:9px;background:#f8fafc;color:#0f172a;font:inherit;font-size:12px;outline:none}.documents-private-ocr-search:focus{border-color:#60a5fa;background:#fff;box-shadow:0 0 0 3px rgba(37,99,235,.1)}.documents-private-ocr-filters{display:flex;align-items:center;gap:6px;flex-wrap:wrap}.documents-private-ocr-filter{min-height:34px;padding:0 11px;border:1px solid #e2e8f0;border-radius:8px;background:#fff;color:#475569;font-size:11px;font-weight:800;cursor:pointer}.documents-private-ocr-filter:hover{border-color:#cbd5e1;background:#f8fafc}.documents-private-ocr-filter.is-active{border-color:#93c5fd;background:#eff6ff;color:#1d4ed8;box-shadow:inset 0 0 0 1px #bfdbfe}' +
      '.documents-private-ocr-pager{display:flex;flex:0 0 auto;align-items:center;gap:7px;margin-left:auto}.documents-private-ocr-page-button{min-height:34px;padding:0 10px;border:1px solid #cbd5e1;border-radius:8px;background:#fff;color:#334155;font-size:11px;font-weight:800;cursor:pointer}.documents-private-ocr-page-button:hover:not(:disabled){border-color:#93c5fd;background:#eff6ff;color:#1d4ed8}.documents-private-ocr-page-button:disabled{cursor:default;opacity:.45}.documents-private-ocr-page-label{min-width:92px;color:#475569;font-size:11px;font-weight:750;text-align:center;white-space:nowrap;font-variant-numeric:tabular-nums}' +
      '.documents-private-ocr-group{display:flex;flex:1 1 auto;flex-direction:column;min-width:0;min-height:0;border:1px solid #dbe3ee;border-radius:13px;background:#fff;overflow:hidden}.documents-private-ocr-group__header{display:flex;flex:0 0 auto;align-items:center;justify-content:space-between;gap:10px;padding:11px 14px;border-bottom:1px solid #e2e8f0;background:#f8fafc}.documents-private-ocr-group__title{font-size:13px;font-weight:900;color:#0f172a}.documents-private-ocr-group__count{color:#475569;font-size:11px;font-weight:750}.documents-private-ocr-list{display:flex;flex:1 1 auto;flex-direction:column;gap:8px;min-width:0;min-height:0;padding:10px;overflow:auto;overflow-x:auto;scrollbar-gutter:stable both-edges;overscroll-behavior:contain}' +
      '.documents-private-ocr-item{display:grid;grid-template-columns:minmax(280px,1fr) minmax(280px,auto);align-items:center;gap:10px 18px;min-width:760px;padding:12px 14px;border:1px solid #e2e8f0;border-left:4px solid #cbd5e1;border-radius:10px;background:#fff;box-shadow:0 2px 7px rgba(15,23,42,.035);transition:border-color .16s ease,box-shadow .16s ease,transform .16s ease}.documents-private-ocr-item:hover{border-color:#bfdbfe;border-left-color:#60a5fa;box-shadow:0 7px 18px rgba(15,23,42,.07);transform:translateY(-1px)}.documents-private-ocr-item--processing,.documents-private-ocr-item--pending{border-left-color:#3b82f6}.documents-private-ocr-item--completed{border-left-color:#10b981}.documents-private-ocr-item--failed,.documents-private-ocr-item--skipped{border-left-color:#ef4444}.documents-private-ocr-item__copy{min-width:0}.documents-private-ocr-item__name{font-size:13px;font-weight:850;line-height:1.35;color:#0f172a;overflow-wrap:anywhere}.documents-private-ocr-item__meta{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:6px;font-size:11px;color:#64748b}.documents-private-ocr-origin{padding:3px 7px;border-radius:999px;background:#eff6ff;color:#1d4ed8;font-size:10px;font-weight:850}.documents-private-ocr-origin--new{background:#eff6ff;color:#1d4ed8}.documents-private-ocr-origin--old{background:#f1f5f9;color:#475569}.documents-private-ocr-item__statuses{display:flex;align-items:center;justify-content:flex-end;gap:6px;flex-wrap:wrap}.documents-private-ocr-item__actions{display:flex;align-items:center;gap:7px;flex-wrap:wrap;grid-column:1/-1;padding-top:2px}.documents-private-ocr-item__progress{display:grid;grid-column:1/-1;grid-template-columns:minmax(0,1fr) auto;align-items:center;gap:8px}.documents-private-ocr-item__progress-track{height:6px;border-radius:999px;background:#e2e8f0;overflow:hidden}.documents-private-ocr-item__progress-bar{height:100%;width:0;border-radius:inherit;background:linear-gradient(90deg,#2563eb,#22c55e);transition:width .25s ease}.documents-private-ocr-item__progress-label{color:#475569;font-size:11px;font-weight:850;font-variant-numeric:tabular-nums;white-space:nowrap}' +
      '.documents-private-ocr-status{flex:0 0 auto;padding:5px 8px;border-radius:999px;background:#f1f5f9;color:#475569;font-size:11px;font-weight:800;white-space:nowrap}.documents-private-ocr-retry,.documents-private-ocr-view,.documents-private-ocr-check{display:inline-flex;align-items:center;justify-content:center;box-sizing:border-box;min-height:30px;padding:0 10px;border:1px solid rgba(37,99,235,.28);border-radius:9px;background:#eff6ff;color:#1d4ed8;font-size:11px;font-weight:800;line-height:1;text-decoration:none;white-space:nowrap;cursor:pointer}.documents-private-ocr-retry:hover:not(:disabled),.documents-private-ocr-view:hover:not(:disabled),.documents-private-ocr-check:hover:not(:disabled){background:#dbeafe}.documents-private-ocr-retry:disabled,.documents-private-ocr-view:disabled,.documents-private-ocr-check:disabled{cursor:wait;opacity:.65}.documents-private-ocr-text,.documents-private-ocr-check-result{grid-column:1/-1;box-sizing:border-box;max-height:320px;padding:11px 12px;border:1px solid rgba(147,197,253,.5);border-radius:12px;background:#f8fafc;color:#0f172a;font:12px/1.55 ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,monospace;white-space:pre-wrap;overflow:auto;overflow-x:auto;scrollbar-gutter:stable;overflow-wrap:anywhere}.documents-private-ocr-check-result--missing{border-color:rgba(248,113,113,.35);background:#fff7f7;color:#991b1b}.documents-private-ocr-text[hidden],.documents-private-ocr-check-result[hidden]{display:none}' +
      '.documents-private-ocr-reason-button{min-height:30px;padding:0 9px;border:1px solid rgba(220,38,38,.22);border-radius:9px;background:#fff7f7;color:#b91c1c;font-size:11px;font-weight:800;white-space:nowrap;cursor:pointer}.documents-private-ocr-reason-button:hover{background:#fff1f2}.documents-private-ocr-reason{grid-column:1/-1;box-sizing:border-box;padding:8px 10px;border:1px solid rgba(248,113,113,.28);border-radius:10px;background:#fff7f7;color:#991b1b;font-size:11px;line-height:1.4;overflow-wrap:anywhere}.documents-private-ocr-reason[hidden]{display:none}' +
      '.documents-private-ocr-status--pending,.documents-private-ocr-status--processing{background:#eff6ff;color:#1d4ed8}.documents-private-ocr-status--completed{background:#ecfdf5;color:#047857}.documents-private-ocr-status--error{background:#fff1f2;color:#b91c1c}.documents-private-ocr-status--unsupported,.documents-private-ocr-status--muted{background:#f8fafc;color:#64748b}.documents-private-ocr-status--missing{background:#fff7ed;color:#c2410c}' +
      ...
      '.documents-private-ocr-modal{padding:0;align-items:stretch}.documents-private-ocr-panel{height:100vh;height:100dvh;max-height:none;border-radius:0}.documents-private-ocr-tabs{padding:6px 8px 0}.documents-private-ocr-tab{flex:1 1 50%;padding:0 8px}.documents-private-ocr-backfill{margin:8px 8px 0;padding:11px}.documents-private-ocr-backfill__top{align-items:stretch;flex-direction:column}.documents-private-ocr-backfill__actions{width:100%;flex-wrap:wrap}.documents-private-ocr-backfill__button{flex:1 1 auto}.documents-private-telegram-audit{margin:8px;min-height:0}.documents-private-telegram-audit__top{align-items:stretch;flex-direction:column}.documents-private-telegram-audit__actions{width:100%;flex-wrap:wrap}.documents-private-telegram-audit__toolbar{align-items:stretch;flex-direction:column;padding:8px}.documents-private-telegram-audit__select{width:100%}' +
      '.documents-private-ocr-monitor{padding:8px}.documents-private-ocr-summary{grid-template-columns:repeat(2,minmax(0,1fr))}.documents-private-ocr-summary__card:first-child{grid-column:1/-1}.documents-private-ocr-toolbar{align-items:stretch;flex-direction:column}.documents-private-ocr-search{width:100%;min-width:0;flex-basis:auto}.documents-private-ocr-pager{width:100%;justify-content:space-between;margin-left:0}.documents-private-ocr-group{min-width:0}.documents-private-ocr-list{padding:8px}.documents-private-ocr-item{grid-template-columns:1fr;min-width:560px}.documents-private-ocr-item__statuses{justify-content:flex-start}.documents-private-ocr-item__actions{grid-column:1}.documents-private-ocr-text,.documents-private-ocr-check-result,.documents-private-ocr-reason{grid-column:1}.documents-private-ocr-status{align-self:flex-start}' +
      '}';
  ...
  function getAdminOcrStatusPresentation(ocr) {
    var status = ocr && ocr.status ? String(ocr.status) : 'pending';
    var presentations = {
      pending: { label: 'В очереди', variant: 'pending' },
      processing: { label: 'Распознаётся', variant: 'processing' },
      completed: { label: 'Готово', variant: 'completed' },
      failed: { label: 'Ошибка', variant: 'error' },
      error: { label: 'Ошибка', variant: 'error' },
      unsupported: { label: 'Формат не поддерживается', variant: 'unsupported' }
    };
    return presentations[status] || { label: status, variant: 'pending' };
  }

  function updateAdminOcrButtonPresentation(cron, backfill) {
    var button = adminElements && adminElements.ocrButton ? adminElements.ocrButton : null;
    if (!button) {
      return;
    }
    var cronState = cron && typeof cron === 'object' ? cron : {};
    var queueState = backfill && typeof backfill === 'object' ? backfill : {};
    var isActive = Boolean(cronState.active);
    var isStarting = Boolean(cronState.installed) && cronState.enabled !== false && !isActive;
    button.classList.toggle('is-running', isActive);
    button.classList.toggle('is-starting', isStarting);
    button.classList.toggle('is-stopped', !isActive && !isStarting);
    button.textContent = isActive
      ? 'OCR · работает'
      : (isStarting ? 'OCR · запускается' : 'OCR · остановлен');
    var queueStatus = String(queueState.status || 'idle');
    var processed = Math.max(0, Number(queueState.processed) || 0);
    var total = Math.max(0, Number(queueState.total) || 0);
    button.title = button.textContent
      + (total > 0 ? ' · очередь ' + processed + ' из ' + total : '')
      + (queueStatus === 'paused' ? ' · старые файлы на паузе' : '');
    button.setAttribute('aria-label', button.title);
  }

  function refreshAdminOcrButtonStatus() {
    if (!state.organization || !adminElements || !adminElements.ocrButton) {
    ...
    return fetch(buildApiUrl('admin_ocr_backfill', {
      ...
      .then(handleResponse)
      .then(function(data) {
        updateAdminOcrButtonPresentation(
          data && data.cron ? data.cron : {},
          data && data.backfill ? data.backfill : {}
        );
      })
      .catch(function(error) {
        var button = adminElements && adminElements.ocrButton ? adminElements.ocrButton : null;
        ...
        button.classList.remove('is-running', 'is-starting');
        button.classList.add('is-stopped');
        button.textContent = 'OCR · нет статуса';
        button.title = error && error.message ? String(error.message) : 'Не удалось проверить фоновый OCR.';
      });
  }

  function openAdminOcrMonitorModal() {
    ensureResponsesStyle();
    var modal = createElement('div', 'documents-brief-modal documents-private-ocr-modal');
    var panel = createElement('div', 'documents-brief-panel documents-private-ocr-panel');
    ...
    var viewTabs = createElement('div', 'documents-private-ocr-tabs');
    var filesTabButton = createElement('button', 'documents-private-ocr-tab is-active', 'OCR-файлы');
    var telegramTabButton = createElement('button', 'documents-private-ocr-tab', 'Telegram JSON');
    var backfillPanel = createElement('section', 'documents-private-ocr-backfill');
    var backfillTop = createElement('div', 'documents-private-ocr-backfill__top');
    var backfillCopy = createElement('div', 'documents-private-ocr-backfill__copy');
    var backfillTitle = createElement('div', 'documents-private-ocr-backfill__title', 'Последовательная очередь OCR и доставки в S3');
    var backfillSummary = createElement('div', 'documents-private-ocr-backfill__summary', 'Загружаю состояние очереди…');
    var backfillActions = createElement('div', 'documents-private-ocr-backfill__actions');
    var backfillPrimaryButton = createElement('button', 'documents-private-ocr-backfill__button', 'Загрузка…');
    var backfillRefreshButton = createElement(
      'button',
      'documents-private-ocr-backfill__button documents-private-ocr-backfill__button--secondary',
      'Обновить'
    );
    var cronInstallButton = createElement(
      'button',
      'documents-private-ocr-backfill__button documents-private-ocr-backfill__button--secondary',
      'Запустить cron'
    );
    var cronRemoveButton = createElement(
      'button',
      'documents-private-ocr-backfill__button documents-private-ocr-backfill__button--secondary',
      'Остановить cron'
    );
    var backfillTrack = createElement('div', 'documents-private-ocr-backfill__track');
    var backfillProgress = createElement('div', 'documents-private-ocr-backfill__progress');
    var backfillStats = createElement('div', 'documents-private-ocr-backfill__stats');
    var cronStatus = createElement('div', 'documents-private-ocr-backfill__cron', 'Проверяю cron…');
    var backfillFailure = createElement('div', 'documents-private-ocr-backfill__failure');
    var snapshotAuditPanel = createElement('section', 'documents-private-telegram-audit');
    var snapshotAuditTop = createElement('div', 'documents-private-telegram-audit__top');
    var snapshotAuditCopy = createElement('div', 'documents-private-telegram-audit__copy');
    var snapshotAuditTitle = createElement('div', 'documents-private-telegram-audit__title', 'Telegram JSON');
    var snapshotAuditSummary = createElement(
      'div',
      'documents-private-telegram-audit__summary',
      'Загружаю состояние полной сверки…'
    );
    var snapshotAuditActions = createElement('div', 'documents-private-telegram-audit__actions');
    var snapshotAuditStartButton = createElement(
      'button',
      'documents-private-ocr-backfill__button',
      'Проверить все JSON'
    );
    var snapshotAuditRetryButton = createElement(
      'button',
      'documents-private-ocr-backfill__button documents-private-ocr-backfill__button--secondary',
      'Повторить ошибки'
    );
    var snapshotAuditTrack = createElement('div', 'documents-private-telegram-audit__track');
    var snapshotAuditProgress = createElement('div', 'documents-private-telegram-audit__progress');
    var snapshotAuditStats = createElement('div', 'documents-private-telegram-audit__stats');
    var snapshotAuditWarning = createElement('div', 'documents-private-telegram-audit__warning');
    var snapshotAuditToolbar = createElement('div', 'documents-private-telegram-audit__toolbar');
    var snapshotAuditSearchInput = createElement('input', 'documents-private-ocr-search');
    var snapshotAuditStatusSelect = createElement('select', 'documents-private-telegram-audit__select');
    var snapshotAuditPager = createElement('div', 'documents-private-ocr-pager');
    var snapshotAuditPreviousButton = createElement('button', 'documents-private-ocr-page-button', 'Назад');
    var snapshotAuditPageLabel = createElement('span', 'documents-private-ocr-page-label', 'Нет данных');
    var snapshotAuditNextButton = createElement('button', 'documents-private-ocr-page-button', 'Вперёд');
    var snapshotAuditTableWrap = createElement('div', 'documents-private-telegram-audit__table-wrap');
    var snapshotAuditTable = createElement('table', 'documents-private-telegram-audit__table');
    var snapshotAuditTableHead = document.createElement('thead');
    var snapshotAuditTableBody = document.createElement('tbody');
    var jobPanel = createElement('div', 'documents-private-ocr-job');
    var jobHead = createElement('div', 'documents-private-ocr-job__head');
    var jobLabel = createElement('div', 'documents-private-ocr-job__label', 'OCR job');
    var jobPercent = createElement('div', 'documents-private-ocr-job__percent', '0%');
    var jobTrack = createElement('div', 'documents-private-ocr-job__track');
    var jobProgress = createElement('div', 'documents-private-ocr-job__progress');
    var jobMeta = createElement('div', 'documents-private-ocr-job__meta');
    var body = createElement('div', 'documents-private-ocr-monitor');
    var ocrPager = createElement('div', 'documents-private-ocr-pager');
    var ocrPreviousButton = createElement('button', 'documents-private-ocr-page-button', 'Назад');
    var ocrPageLabel = createElement('span', 'documents-private-ocr-page-label', 'Нет данных');
    var ocrNextButton = createElement('button', 'documents-private-ocr-page-button', 'Вперёд');
    var backfillTimerId = null;
    var backfillRequestBusy = false;
    var backfillState = null;
    var cronState = null;
    var snapshotAuditRequestBusy = false;
    var snapshotAuditState = null;
    var activeOcrView = 'files';
    var ocrItemsOffset = 0;
    var ocrItemsTotal = 0;
    var snapshotAuditItemsOffset = 0;
    var snapshotAuditItemsTotal = 0;
    var snapshotAuditSearchQuery = '';
    var snapshotAuditStatusFilter = 'all';
    var searchDebounceId = null;
    var snapshotAuditSearchDebounceId = null;

    titleWrap.appendChild(createElement('div', 'documents-brief-title', 'Управление OCR'));
    titleWrap.appendChild(createElement(
      'div',
      'documents-brief-subtitle',
      'Контроль распознавания файлов и сохранения текста в telegramId.json всех участников задачи.'
    ));

    function closeOcrMonitor() {
      if (backfillTimerId !== null) {
        window.clearInterval(backfillTimerId);
        backfillTimerId = null;
      }
      if (searchDebounceId !== null) {
        window.clearTimeout(searchDebounceId);
        searchDebounceId = null;
      }
      if (snapshotAuditSearchDebounceId !== null) {
        window.clearTimeout(snapshotAuditSearchDebounceId);
        snapshotAuditSearchDebounceId = null;
      }
      document.removeEventListener('keydown', handleOcrEscape, true);
      if (modal.parentNode) {
        modal.parentNode.removeChild(modal);
      }
    }

    function handleOcrEscape(event) {
      if (event.key !== 'Escape') {
        return;
      }
    ...
    closeButton.addEventListener('click', closeOcrMonitor);
    ...
    function requestOcrBackfill(operation) {
      var isStatus = operation === 'status';
      var payload = {
        action: 'admin_ocr_backfill',
        operation: operation,
        organization: state.organization,
        compact: 1,
        itemsOffset: ocrItemsOffset,
        itemsLimit: OCR_ADMIN_PAGE_SIZE,
        itemsSearch: searchQuery,
      ...
      return fetch(
        buildApiUrl(
          ...
          isStatus
            ? {
                organization: state.organization,
                compact: '1',
                itemsOffset: String(ocrItemsOffset),
                itemsLimit: String(OCR_ADMIN_PAGE_SIZE),
                itemsSearch: searchQuery,
                itemsFilter: activeFilter,
                cacheBust: String(Date.now())
              }
        ...
        ),
        {
          ...
          headers: isStatus ? undefined : { 'Content-Type': 'application/json' },
          credentials: 'same-origin',
          cache: 'no-store',
          body: isStatus ? undefined : JSON.stringify(payload)
        }
      ).then(handleResponse);
    }

    function requestSnapshotAudit(operation) {
      var isStatus = operation === 'status';
      var payload = {
        action: 'admin_telegram_snapshot_audit',
        operation: operation,
        organization: state.organization,
        compact: 1,
        itemsOffset: snapshotAuditItemsOffset,
        itemsLimit: OCR_ADMIN_PAGE_SIZE,
        itemsSearch: snapshotAuditSearchQuery,
        itemsFilter: snapshotAuditStatusFilter
      };
      mergeTelegramUserId(payload);
      return fetch(
        buildApiUrl(
          'admin_telegram_snapshot_audit',
          isStatus
            ? {
                organization: state.organization,
                compact: '1',
                itemsOffset: String(snapshotAuditItemsOffset),
                itemsLimit: String(OCR_ADMIN_PAGE_SIZE),
                itemsSearch: snapshotAuditSearchQuery,
                itemsFilter: snapshotAuditStatusFilter,
                cacheBust: String(Date.now())
    ...
    function snapshotAuditStatusLabel(status) {
      var labels = {
        pending: 'Ожидает',
        processing: 'Проверяется',
        valid: 'Актуален',
        reconciled: 'Исправлен',
        failed: 'Ошибка'
      };
      return labels[status] || status || 'Ожидает';
    }

    function updatePageControls(total, offset, count, label, previousButton, nextButton, busy) {
      var safeTotal = Math.max(0, Number(total) || 0);
      var safeOffset = Math.max(0, Number(offset) || 0);
      var safeCount = Math.max(0, Number(count) || 0);
      label.textContent = safeTotal > 0 && safeCount > 0
        ? (safeOffset + 1) + '–' + Math.min(safeTotal, safeOffset + safeCount) + ' из ' + safeTotal
        : (safeTotal > 0 ? '0 из ' + safeTotal : 'Нет данных');
      previousButton.disabled = busy || safeOffset === 0;
      nextButton.disabled = busy || safeOffset + safeCount >= safeTotal || safeCount === 0;
    }

    function renderSnapshotAudit(nextState) {
      snapshotAuditState = nextState && typeof nextState === 'object' ? nextState : {};
      var status = String(snapshotAuditState.status || 'idle');
      var total = Math.max(0, Number(snapshotAuditState.total) || 0);
      var processed = Math.max(0, Number(snapshotAuditState.processed) || 0);
      var valid = Math.max(0, Number(snapshotAuditState.valid) || 0);
      var reconciled = Math.max(0, Number(snapshotAuditState.reconciled) || 0);
      var failed = Math.max(0, Number(snapshotAuditState.failed) || 0);
      var created = Math.max(0, Number(snapshotAuditState.created) || 0);
      var added = Math.max(0, Number(snapshotAuditState.missingTasksAdded) || 0);
      var removed = Math.max(0, Number(snapshotAuditState.staleTasksRemoved) || 0);
      var progress = Math.max(0, Math.min(100, Number(snapshotAuditState.progress) || 0));
      var currentTelegramId = snapshotAuditState.currentTelegramId
        ? String(snapshotAuditState.currentTelegramId)
        : '';

      if (status === 'running') {
        snapshotAuditSummary.textContent = 'Проверено ' + processed + ' из ' + total
          + (currentTelegramId ? ' · сейчас ' + currentTelegramId + '.json' : '') + '.';
      } else if (status === 'completed' && total > 0) {
        snapshotAuditSummary.textContent = 'Проверка завершена: ' + processed + ' из ' + total + ' JSON.';
      } else {
        snapshotAuditSummary.textContent = 'Создаёт отсутствующие JSON и сверяет полный состав задач каждого Telegram-пользователя.';
      }
      snapshotAuditProgress.style.width = progress + '%';
      snapshotAuditStats.textContent = '';
      [
        'Актуальны: ' + valid,
        'Исправлены: ' + reconciled,
        'Созданы: ' + created,
        'Добавлено задач: ' + added,
        'Удалено лишних: ' + removed,
        'Ошибки: ' + failed
      ].forEach(function(label) {
        snapshotAuditStats.appendChild(createElement('span', '', label));
      });

      var inventoryWarning = snapshotAuditState.inventoryWarning
        ? String(snapshotAuditState.inventoryWarning)
        : '';
      snapshotAuditWarning.hidden = !inventoryWarning;
      snapshotAuditWarning.textContent = inventoryWarning
        ? 'Не удалось полностью прочитать список S3: ' + inventoryWarning
        : '';
      snapshotAuditStartButton.disabled = snapshotAuditRequestBusy || status === 'running';
      snapshotAuditStartButton.textContent = status === 'running'
        ? 'Проверка идёт'
        : (status === 'completed' ? 'Проверить заново' : 'Проверить все JSON');
      snapshotAuditRetryButton.hidden = failed === 0;
      snapshotAuditRetryButton.disabled = snapshotAuditRequestBusy || failed === 0 || status === 'running';

      var items = Array.isArray(snapshotAuditState.items) ? snapshotAuditState.items : [];
      snapshotAuditItemsTotal = Math.max(0, Number(snapshotAuditState.itemsTotal) || 0);
      snapshotAuditItemsOffset = Math.max(0, Number(snapshotAuditState.itemsOffset) || 0);
      updatePageControls(
        snapshotAuditItemsTotal,
        snapshotAuditItemsOffset,
        items.length,
        snapshotAuditPageLabel,
        snapshotAuditPreviousButton,
        snapshotAuditNextButton,
        snapshotAuditRequestBusy
      );
      if (activeOcrView !== 'telegram') {
        return;
      }

      var previousScrollTop = snapshotAuditTableWrap.scrollTop;
      var previousScrollLeft = snapshotAuditTableWrap.scrollLeft;
      snapshotAuditTableBody.textContent = '';
      if (!items.length) {
        var emptyRow = document.createElement('tr');
        var emptyCell = document.createElement('td');
        emptyCell.colSpan = 6;
        emptyCell.textContent = snapshotAuditSearchQuery || snapshotAuditStatusFilter !== 'all'
          ? 'По выбранным условиям пользователей нет.'
          : (status === 'completed'
            ? 'Пользователи с Telegram ID не найдены.'
            : 'Сверка ещё не запускалась.');
        emptyRow.appendChild(emptyCell);
        snapshotAuditTableBody.appendChild(emptyRow);
      } else {
        items.forEach(function(item) {
          var row = document.createElement('tr');
          var userCell = document.createElement('td');
          var userId = item && item.telegramId ? String(item.telegramId) : '—';
          userCell.appendChild(createElement('div', 'documents-private-telegram-audit__user', userId + '.json'));
          if (item && item.displayName) {
            userCell.appendChild(createElement(
              'div',
              'documents-private-telegram-audit__name',
              String(item.displayName)
            ));
          }
          row.appendChild(userCell);
          [
            Math.max(0, Number(item && item.actualTasksBefore) || 0),
            Math.max(0, Number(item && item.expectedTasksCount) || 0),
            Math.max(0, Number(item && item.missingTasksAdded) || 0),
            Math.max(0, Number(item && item.staleTasksRemoved) || 0)
          ...
          var itemStatus = item && item.status ? String(item.status) : 'pending';
          statusCell.appendChild(createElement(
            'span',
            'documents-private-telegram-audit__status documents-private-telegram-audit__status--' + itemStatus,
            snapshotAuditStatusLabel(itemStatus)
          ));
          if (item && item.error) {
            statusCell.appendChild(createElement(
              'div',
              'documents-private-telegram-audit__error',
              String(item.error)
            ));
          }
          row.appendChild(statusCell);
          snapshotAuditTableBody.appendChild(row);
        });
      }
      snapshotAuditTableWrap.scrollTop = previousScrollTop;
      snapshotAuditTableWrap.scrollLeft = previousScrollLeft;
    }

    function updateSnapshotAudit(operation, shouldNotify) {
      if (snapshotAuditRequestBusy || !modal.isConnected) {
        return Promise.resolve();
      }
      snapshotAuditRequestBusy = true;
      snapshotAuditStartButton.disabled = true;
      snapshotAuditRetryButton.disabled = true;
      snapshotAuditPreviousButton.disabled = true;
      snapshotAuditNextButton.disabled = true;
      return requestSnapshotAudit(operation)
        .then(function(data) {
          snapshotAuditRequestBusy = false;
          if (data && data.cron && typeof data.cron === 'object') {
            cronState = data.cron;
            renderOcrBackfill(backfillState, cronState);
          }
          renderSnapshotAudit(data && data.snapshotAudit ? data.snapshotAudit : {});
          if (shouldNotify && data && data.message) {
            showMessage('success', String(data.message));
          }
        })
        .catch(function(error) {
          snapshotAuditRequestBusy = false;
          if (modal.isConnected) {
            renderSnapshotAudit(snapshotAuditState);
          }
          if (shouldNotify) {
            showMessage(
              'error',
              error && error.message ? error.message : 'Не удалось управлять проверкой Telegram JSON.'
            );
          }
        });
    }

    function renderOcrBackfill(nextState, nextCronState) {
      backfillState = nextState && typeof nextState === 'object' ? nextState : {};
      if (nextCronState && typeof nextCronState === 'object') {
        cronState = nextCronState;
      }
      if (Array.isArray(backfillState.items) && Array.isArray(allFiles)) {
        ocrItemsTotal = Math.max(0, Number(backfillState.itemsTotal) || 0);
        ocrItemsOffset = Math.max(0, Number(backfillState.itemsOffset) || 0);
        allFiles = backfillState.items.map(function(item) {
          var queueStatus = item && item.status ? String(item.status).toLowerCase() : 'pending';
          var completed = queueStatus === 'completed';
          var failed = queueStatus === 'failed';
          var processing = queueStatus === 'processing';
          var skipped = queueStatus === 'skipped';
          var deliveryPending = Boolean(item && item.deliveryPending);
          var ocrStatus = completed || deliveryPending
            ? 'completed'
            : (failed ? 'failed' : (processing ? 'processing' : (skipped ? 'unsupported' : 'pending')));
          var ocr = {
            status: ocrStatus,
            error: item && item.error ? String(item.error) : ''
          };
          if (completed || deliveryPending) {
            ocr.snapshotStatus = completed ? 'synced' : 'error';
            ocr.snapshotDeliveryVersion = completed ? OCR_SNAPSHOT_DELIVERY_VERSION : 0;
            ocr.snapshotUsersCount = Math.max(0, Number(item.participantsTotal) || 0);
          }
          return {
            name: item && (item.originalName || item.storedName)
              ? String(item.originalName || item.storedName)
              : 'Файл',
            taskLabel: item && item.taskLabel ? String(item.taskLabel) : String(item && item.documentId || '—'),
            documentId: item && item.documentId ? String(item.documentId) : '',
            storedName: item && item.storedName ? String(item.storedName) : '',
            fileRef: null,
            size: Math.max(0, Number(item && item.size) || 0),
            ocr: ocr,
            textLength: 0,
            error: ocr.error,
            snapshotStatus: completed ? 'synced' : (deliveryPending ? 'error' : ''),
            snapshotDeliveryVersion: completed ? OCR_SNAPSHOT_DELIVERY_VERSION : 0,
            snapshotUsersCount: completed ? Math.max(0, Number(item.participantsTotal) || 0) : 0,
            snapshotError: ocr.error,
            participantsTotal: Math.max(0, Number(item && item.participantsTotal) || 0),
            presentCount: Math.max(0, Number(item && item.presentCount) || 0),
            missingCount: Math.max(0, Number(item && item.missingCount) || 0),
            queueStatus: queueStatus,
            deliveryPending: deliveryPending,
            origin: item && item.origin === 'live_upload' ? 'live_upload' : 'old_backfill',
            fullReprocess: Boolean(item && item.fullReprocess),
            jobStatus: item && item.jobStatus ? String(item.jobStatus).toLowerCase() : '',
            progress: Math.max(0, Math.min(100, Number(item && item.progress) || 0)),
            currentPage: Math.max(0, Number(item && item.currentPage) || 0),
            totalPages: Math.max(0, Number(item && item.totalPages) || 0)
          };
        });
        ...
        if (activeOcrView === 'files') {
          renderFileList();
        }
      }
      var status = String(backfillState.status || 'idle');
      var total = Math.max(0, Number(backfillState.total) || 0);
      var processed = Math.max(0, Number(backfillState.processed) || 0);
      var completed = Math.max(0, Number(backfillState.completed) || 0);
      var failed = Math.max(0, Number(backfillState.failed) || 0);
      var skipped = Math.max(0, Number(backfillState.skipped) || 0);
      var progress = Math.max(0, Math.min(100, Number(backfillState.progress) || 0));
      var currentFile = backfillState.currentFile ? String(backfillState.currentFile) : '';
      var currentJob = backfillState.currentJob && typeof backfillState.currentJob === 'object'
        ? backfillState.currentJob
        : null;
      var summary = '';

      if (status === 'running') {
        summary = 'Обработано ' + processed + ' из ' + total + ' · общий прогресс ' + progress.toFixed(1).replace('.0', '') + '%.';
      } else if (status === 'paused') {
        summary = 'Старые файлы приостановлены: ' + processed + ' из ' + total
          + ' · ' + progress.toFixed(1).replace('.0', '')
          + '%. Новые файлы продолжают обрабатываться автоматически.';
      } else if (status === 'completed' && total > 0) {
        summary = 'Обработка завершена: ' + processed + ' из ' + total + '.';
      } else {
        summary = 'Очередь ещё не создана. Полный запуск заново отправит каждый существующий файл на новую OCR job и перезапишет текст участников.';
      }
      if (currentFile) {
        summary += ' Сейчас: ' + currentFile;
      }
      backfillSummary.textContent = summary;
      backfillProgress.style.width = progress + '%';
      if ((status === 'running' || status === 'paused') && currentJob) {
        var jobProgressValue = Math.max(0, Math.min(100, Number(currentJob.progress) || 0));
        var currentPage = Math.max(0, Number(currentJob.currentPage) || 0);
        var totalPages = Math.max(0, Number(currentJob.totalPages) || 0);
        var jobStatus = String(currentJob.jobStatus || 'queued');
        jobPanel.hidden = false;
        jobLabel.textContent = (currentFile || 'Текущий файл') + ' · '
          + (currentJob.deliveryPending
            ? 'OCR готов, доставка в S3'
            : (jobStatus === 'running' ? 'распознаётся' : 'в очереди'));
        jobPercent.textContent = Math.round(jobProgressValue) + '%';
        jobProgress.style.width = jobProgressValue + '%';
        jobMeta.textContent = totalPages > 0
          ? 'Страница ' + Math.min(currentPage, totalPages) + ' из ' + totalPages
          : (currentPage > 0 ? 'Обработана страница ' + currentPage : 'Ожидание данных о страницах');
      } else {
        jobPanel.hidden = true;
        jobProgress.style.width = '0%';
        jobMeta.textContent = '';
      }
      backfillStats.textContent = '';
      [
        'Готово: ' + completed,
        'Ошибки: ' + failed,
        'Пропущено: ' + skipped
      ].filter(Boolean).forEach(function(label) {
        backfillStats.appendChild(createElement('span', '', label));
      });

      var cronInstalled = Boolean(cronState && cronState.installed);
      var cronActive = Boolean(cronState && cronState.active);
      var cronEnabled = !cronState || cronState.enabled !== false;
      var cronSupported = !cronState || cronState.supported !== false;
      var cronLastRun = cronState && cronState.lastRunAt ? Date.parse(String(cronState.lastRunAt)) : 0;
      var cronLastRunLabel = Number.isFinite(cronLastRun) && cronLastRun > 0
        ? new Date(cronLastRun).toLocaleString('ru-RU')
        : '';
      var cronHasError = cronEnabled && (!cronSupported || Boolean(cronState && cronState.error));
      cronStatus.className = 'documents-private-ocr-backfill__cron'
        + (cronActive ? ' is-active' : (cronHasError ? ' is-error' : ''));
      if (!cronEnabled) {
        cronStatus.textContent = 'Cron остановлен: очередь сохранена и продолжится после повторного запуска.';
      } else if (!cronSupported) {
        cronStatus.textContent = 'Cron недоступен: '
          + String(cronState && cronState.error || 'сервер запретил команду crontab.');
      } else if (cronActive) {
        cronStatus.textContent = 'Cron работает каждую минуту'
          + (cronLastRunLabel ? ' · последний запуск ' + cronLastRunLabel : '') + '.';
      } else if (cronInstalled && cronEnabled) {
        cronStatus.textContent = 'Cron установлен, ожидается подтверждение первого фонового запуска'
          + (cronLastRunLabel ? ' · последний известный запуск ' + cronLastRunLabel : '') + '.';
      } else {
        cronStatus.textContent = 'Cron не установлен. Запустите cron, чтобы очередь работала после закрытия сайта.';
      }
      updateAdminOcrButtonPresentation(cronState, backfillState);

      var failures = Array.isArray(backfillState.recentFailures) ? backfillState.recentFailures : [];
      var lastFailure = failures.length ? failures[failures.length - 1] : null;
      if (lastFailure && lastFailure.error) {
        backfillFailure.hidden = false;
        backfillFailure.textContent = 'Последняя ошибка'
          + (lastFailure.file ? ' · ' + lastFailure.file : '')
          + ': '
          + lastFailure.error;
      } else {
        backfillFailure.hidden = true;
        backfillFailure.textContent = '';
      }

      backfillPrimaryButton.disabled = backfillRequestBusy;
      if (status === 'running') {
        backfillPrimaryButton.textContent = 'Пауза старых файлов';
        backfillPrimaryButton.dataset.operation = 'pause';
      } else if (status === 'paused') {
        backfillPrimaryButton.textContent = 'Продолжить';
        backfillPrimaryButton.dataset.operation = 'resume';
      } else {
        backfillPrimaryButton.textContent = status === 'completed'
          ? 'Перезапустить все старые файлы'
          : 'Полный OCR старых файлов';
        backfillPrimaryButton.dataset.operation = 'start';
      }
      backfillRefreshButton.disabled = backfillRequestBusy;
      cronInstallButton.disabled = backfillRequestBusy || !cronSupported;
      cronInstallButton.textContent = cronInstalled && cronEnabled
        ? 'Перезапустить cron'
        : 'Запустить cron';
      cronRemoveButton.disabled = backfillRequestBusy || (!cronInstalled && !cronEnabled);
    }

    function updateOcrBackfill(operation, showError) {
      if (backfillRequestBusy || !modal.isConnected) {
        return Promise.resolve();
      }
      backfillRequestBusy = true;
      backfillPrimaryButton.disabled = true;
      backfillRefreshButton.disabled = true;
      cronInstallButton.disabled = true;
      cronRemoveButton.disabled = true;
      ocrPreviousButton.disabled = true;
      ocrNextButton.disabled = true;
      return requestOcrBackfill(operation)
        .then(function(data) {
          backfillRequestBusy = false;
          renderOcrBackfill(
            data && data.backfill ? data.backfill : {},
            data && data.cron ? data.cron : cronState
          );
          if (data && data.snapshotAudit) {
            renderSnapshotAudit(data.snapshotAudit);
          }
          if (showError && data && data.message) {
            showMessage('success', String(data.message));
          }
        })
        .catch(function(error) {
          ...
          if (showError) {
            showMessage('error', error && error.message ? error.message : 'Не удалось управлять OCR-очередью.');
          }
        });
    }

    function pollOcrBackfill() {
      if (!modal.isConnected) {
        return;
      }
      if (activeOcrView === 'telegram') {
        if (!snapshotAuditRequestBusy) {
          updateSnapshotAudit('status', false);
        }
        return;
      }
      if (!backfillRequestBusy) {
        updateOcrBackfill('status', false);
      }
    }

    function scheduleOcrPageRefresh() {
      if (searchDebounceId !== null) {
        window.clearTimeout(searchDebounceId);
      }
      searchDebounceId = window.setTimeout(function refreshPage() {
        if (!modal.isConnected) {
          return;
        }
        if (backfillRequestBusy) {
          searchDebounceId = window.setTimeout(refreshPage, 150);
          return;
        }
        searchDebounceId = null;
        updateOcrBackfill('status', false);
    ...
    function scheduleSnapshotAuditPageRefresh() {
      if (snapshotAuditSearchDebounceId !== null) {
        window.clearTimeout(snapshotAuditSearchDebounceId);
      }
      snapshotAuditSearchDebounceId = window.setTimeout(function refreshPage() {
        if (!modal.isConnected) {
          return;
        }
        if (snapshotAuditRequestBusy) {
          snapshotAuditSearchDebounceId = window.setTimeout(refreshPage, 150);
          return;
        }
        snapshotAuditSearchDebounceId = null;
        updateSnapshotAudit('status', false);
    ...
    function setActiveOcrView(view) {
      activeOcrView = view === 'telegram' ? 'telegram' : 'files';
      var showFiles = activeOcrView === 'files';
      filesTabButton.classList.toggle('is-active', showFiles);
      telegramTabButton.classList.toggle('is-active', !showFiles);
      filesTabButton.setAttribute('aria-selected', showFiles ? 'true' : 'false');
      telegramTabButton.setAttribute('aria-selected', showFiles ? 'false' : 'true');
      backfillPanel.hidden = !showFiles;
      body.hidden = !showFiles;
      snapshotAuditPanel.hidden = showFiles;
      if (showFiles) {
        if (snapshotAuditSearchDebounceId !== null) {
          window.clearTimeout(snapshotAuditSearchDebounceId);
          snapshotAuditSearchDebounceId = null;
        ...
        renderSnapshotAudit(snapshotAuditState || {});
        scheduleSnapshotAuditPageRefresh();
      }
    }

    filesTabButton.type = 'button';
    telegramTabButton.type = 'button';
    filesTabButton.setAttribute('role', 'tab');
    telegramTabButton.setAttribute('role', 'tab');
    filesTabButton.setAttribute('aria-selected', 'true');
    telegramTabButton.setAttribute('aria-selected', 'false');
    filesTabButton.addEventListener('click', function() {
      if (activeOcrView !== 'files') {
        setActiveOcrView('files');
      }
    });
    telegramTabButton.addEventListener('click', function() {
      if (activeOcrView !== 'telegram') {
        setActiveOcrView('telegram');
      }
    });

    backfillPrimaryButton.type = 'button';
    backfillPrimaryButton.addEventListener('click', function() {
      var operation = backfillPrimaryButton.dataset.operation || 'start';
      if (operation === 'start') {
        var confirmed = window.confirm(
          'Полностью запустить OCR старых файлов заново?\n\n'
          + 'Каждый существующий файл получит новую OCR job, даже если текст уже сохранён. '
          + 'После успеха новый текст перезапишет OCR во всех telegramId.json участников задачи.'
        );
        if (!confirmed) {
          return;
        }
      }
      updateOcrBackfill(operation, true).then(function() {
        if (modal.isConnected && operation !== 'pause') {
          pollOcrBackfill();
        }
      });
    });
    backfillRefreshButton.type = 'button';
    backfillRefreshButton.addEventListener('click', function() {
      updateOcrBackfill('status', true);
    });
    cronInstallButton.type = 'button';
    cronInstallButton.addEventListener('click', function() {
      updateOcrBackfill('cron_install', true);
    });
    cronRemoveButton.type = 'button';
    cronRemoveButton.addEventListener('click', function() {
      if (!window.confirm('Остановить фоновый OCR и проверку Telegram JSON? Очереди сохранятся, остальные cron-задачи сервера не изменятся.')) {
        return;
      }
      updateOcrBackfill('cron_remove', true);
    });
    snapshotAuditStartButton.type = 'button';
    snapshotAuditStartButton.addEventListener('click', function() {
      updateSnapshotAudit('start', true);
    });
    snapshotAuditRetryButton.type = 'button';
    snapshotAuditRetryButton.addEventListener('click', function() {
      updateSnapshotAudit('retry_failed', true);
    });

    backfillCopy.appendChild(backfillTitle);
    backfillCopy.appendChild(backfillSummary);
    backfillActions.appendChild(backfillPrimaryButton);
    backfillActions.appendChild(cronInstallButton);
    backfillActions.appendChild(cronRemoveButton);
    backfillActions.appendChild(backfillRefreshButton);
    backfillTop.appendChild(backfillCopy);
    backfillTop.appendChild(backfillActions);
    backfillTrack.appendChild(backfillProgress);
    backfillPanel.appendChild(backfillTop);
    backfillPanel.appendChild(backfillTrack);
    backfillPanel.appendChild(backfillStats);
    backfillPanel.appendChild(cronStatus);
    jobHead.appendChild(jobLabel);
    jobHead.appendChild(jobPercent);
    jobTrack.appendChild(jobProgress);
    jobPanel.appendChild(jobHead);
    jobPanel.appendChild(jobTrack);
    jobPanel.appendChild(jobMeta);
    jobPanel.hidden = true;
    backfillPanel.appendChild(jobPanel);
    backfillFailure.hidden = true;
    backfillPanel.appendChild(backfillFailure);

    snapshotAuditCopy.appendChild(snapshotAuditTitle);
    snapshotAuditCopy.appendChild(snapshotAuditSummary);
    snapshotAuditActions.appendChild(snapshotAuditStartButton);
    snapshotAuditActions.appendChild(snapshotAuditRetryButton);
    snapshotAuditTop.appendChild(snapshotAuditCopy);
    snapshotAuditTop.appendChild(snapshotAuditActions);
    snapshotAuditTrack.appendChild(snapshotAuditProgress);
    var snapshotAuditHeaderRow = document.createElement('tr');
    ['Пользователь', 'Было задач', 'Должно быть', 'Добавлено', 'Удалено', 'Статус'].forEach(function(label) {
      var headerCell = document.createElement('th');
      headerCell.scope = 'col';
      headerCell.textContent = label;
      snapshotAuditHeaderRow.appendChild(headerCell);
    });
    snapshotAuditTableHead.appendChild(snapshotAuditHeaderRow);
    snapshotAuditTable.appendChild(snapshotAuditTableHead);
    snapshotAuditTable.appendChild(snapshotAuditTableBody);
    snapshotAuditTableWrap.tabIndex = 0;
    snapshotAuditTableWrap.setAttribute(
      'aria-label',
      'Результаты полной сверки Telegram JSON и задач пользователей'
    );
    snapshotAuditTableWrap.appendChild(snapshotAuditTable);
    snapshotAuditSearchInput.type = 'search';
    snapshotAuditSearchInput.placeholder = 'Поиск по Telegram ID или имени';
    snapshotAuditSearchInput.setAttribute('aria-label', 'Поиск Telegram JSON');
    snapshotAuditSearchInput.addEventListener('input', function() {
      snapshotAuditSearchQuery = snapshotAuditSearchInput.value.trim().toLowerCase();
      snapshotAuditItemsOffset = 0;
      scheduleSnapshotAuditPageRefresh();
    });
    [
      { value: 'all', label: 'Все статусы' },
      { value: 'pending', label: 'Ожидают' },
      { value: 'processing', label: 'Проверяются' },
      { value: 'valid', label: 'Актуальны' },
      { value: 'reconciled', label: 'Исправлены' },
      { value: 'failed', label: 'Ошибки' }
    ].forEach(function(optionData) {
      var option = document.createElement('option');
      option.value = optionData.value;
      option.textContent = optionData.label;
      snapshotAuditStatusSelect.appendChild(option);
    });
    snapshotAuditStatusSelect.setAttribute('aria-label', 'Фильтр Telegram JSON по статусу');
    snapshotAuditStatusSelect.addEventListener('change', function() {
      snapshotAuditStatusFilter = snapshotAuditStatusSelect.value || 'all';
      snapshotAuditItemsOffset = 0;
      scheduleSnapshotAuditPageRefresh();
    });
    snapshotAuditPreviousButton.type = 'button';
    snapshotAuditNextButton.type = 'button';
    snapshotAuditPreviousButton.addEventListener('click', function() {
      snapshotAuditItemsOffset = Math.max(0, snapshotAuditItemsOffset - OCR_ADMIN_PAGE_SIZE);
      snapshotAuditTableWrap.scrollTop = 0;
      updateSnapshotAudit('status', false);
    });
    snapshotAuditNextButton.addEventListener('click', function() {
      snapshotAuditItemsOffset += OCR_ADMIN_PAGE_SIZE;
      snapshotAuditTableWrap.scrollTop = 0;
      updateSnapshotAudit('status', false);
    });
    snapshotAuditPager.appendChild(snapshotAuditPreviousButton);
    snapshotAuditPager.appendChild(snapshotAuditPageLabel);
    snapshotAuditPager.appendChild(snapshotAuditNextButton);
    snapshotAuditToolbar.appendChild(snapshotAuditSearchInput);
    snapshotAuditToolbar.appendChild(snapshotAuditStatusSelect);
    snapshotAuditToolbar.appendChild(snapshotAuditPager);
    snapshotAuditPanel.appendChild(snapshotAuditTop);
    snapshotAuditPanel.appendChild(snapshotAuditTrack);
    snapshotAuditPanel.appendChild(snapshotAuditStats);
    snapshotAuditWarning.hidden = true;
    snapshotAuditPanel.appendChild(snapshotAuditWarning);
    snapshotAuditPanel.appendChild(snapshotAuditToolbar);
    snapshotAuditPanel.appendChild(snapshotAuditTableWrap);

    var allFiles = [];
    var activeFilter = 'all';
    var searchQuery = '';
    var summaryValues = {};
    var summary = createElement('div', 'documents-private-ocr-summary');
    var toolbar = createElement('div', 'documents-private-ocr-toolbar');
    var searchInput = createElement('input', 'documents-private-ocr-search');
    var filters = createElement('div', 'documents-private-ocr-filters');
    var section = createElement('section', 'documents-private-ocr-group');
    var heading = createElement('div', 'documents-private-ocr-group__header');
    var visibleCount = createElement('div', 'documents-private-ocr-group__count');
    var list = createElement('div', 'documents-private-ocr-list');

    function getFileDashboardState(file) {
      var rawStatus = file && file.ocr && file.ocr.status
        ? String(file.ocr.status).toLowerCase()
        : '';
      var snapshotStatus = file && file.snapshotStatus
        ? String(file.snapshotStatus).toLowerCase()
        : '';
      var completed = rawStatus === 'completed';
      var synced = completed
        && snapshotStatus === 'synced'
        && file.snapshotDeliveryVersion >= OCR_SNAPSHOT_DELIVERY_VERSION;
      var processing = rawStatus === 'pending' || rawStatus === 'processing';
      var attention = !rawStatus
        || rawStatus === 'failed'
        || rawStatus === 'error'
      ...
      return {
        ...
        attention: attention,
        ready: completed && synced
      };
    }

    function addSummaryCard(key, label, modifier) {
      var card = createElement(
        'div',
        'documents-private-ocr-summary__card' + (modifier ? ' documents-private-ocr-summary__card--' + modifier : '')
      );
      var value = createElement('div', 'documents-private-ocr-summary__value', '0');
      card.appendChild(value);
      card.appendChild(createElement('div', 'documents-private-ocr-summary__label', label));
      summaryValues[key] = value;
      summary.appendChild(card);
    }

    function updateDashboardStats() {
      var stateTotal = backfillState && Number(backfillState.total);
      var completed = backfillState && Number(backfillState.completed);
      var failed = backfillState && Number(backfillState.failed);
      var skipped = backfillState && Number(backfillState.skipped);
      var pending = backfillState && Number(backfillState.pending);
      var deliveryPending = backfillState && Number(backfillState.deliveryPending);
      var totals = Number.isFinite(stateTotal)
        ? {
            total: Math.max(0, stateTotal),
            processing: Math.max(0, Number.isFinite(pending) ? pending : 0),
            completed: Math.max(0, Number.isFinite(completed) ? completed : 0),
            attention: Math.max(0, (Number.isFinite(failed) ? failed : 0)
              + (Number.isFinite(skipped) ? skipped : 0)
              + (Number.isFinite(deliveryPending) ? deliveryPending : 0)),
            synced: Math.max(0, Number.isFinite(completed) ? completed : 0)
          }
        : {
      ...
      if (!Number.isFinite(stateTotal)) {
        allFiles.forEach(function(file) {
          var fileState = getFileDashboardState(file);
          totals.processing += fileState.processing ? 1 : 0;
          totals.completed += fileState.completed ? 1 : 0;
          totals.attention += fileState.attention ? 1 : 0;
          totals.synced += fileState.synced ? 1 : 0;
        });
      }
      Object.keys(summaryValues).forEach(function(key) {
        summaryValues[key].textContent = String(totals[key] || 0);
      });
    }

    function updateFileStatusBadges(file, ocrBadge, s3Badge) {
      var fileState = getFileDashboardState(file);
      var ocrStatus = file.ocr
        ? getAdminOcrStatusPresentation(file.ocr)
        : { label: 'Не запускался', variant: 'muted' };
      ocrBadge.className = 'documents-private-ocr-status documents-private-ocr-status--' + ocrStatus.variant;
      ocrBadge.textContent = 'OCR: ' + ocrStatus.label
        + (fileState.completed && file.textLength ? ' · ' + file.textLength + ' симв.' : '');

      if (!fileState.completed) {
        s3Badge.className = 'documents-private-ocr-status documents-private-ocr-status--muted';
        s3Badge.textContent = 'S3: ожидает OCR';
      } else if (fileState.synced) {
        s3Badge.className = 'documents-private-ocr-status documents-private-ocr-status--completed';
        s3Badge.textContent = 'S3: сохранено'
          + (file.snapshotUsersCount ? ' · ' + file.snapshotUsersCount : '');
      } else if (fileState.snapshotStatus === 'partial') {
        s3Badge.className = 'documents-private-ocr-status documents-private-ocr-status--missing';
        s3Badge.textContent = 'S3: сохранено частично';
      } else if (fileState.snapshotStatus === 'error') {
        s3Badge.className = 'documents-private-ocr-status documents-private-ocr-status--error';
        s3Badge.textContent = 'S3: ошибка';
      } else {
        s3Badge.className = 'documents-private-ocr-status documents-private-ocr-status--missing';
        s3Badge.textContent = 'S3: не подтверждено';
      }
    }

    function createFileItem(file) {
      var fileState = getFileDashboardState(file);
      var queueStatus = file && file.queueStatus ? String(file.queueStatus).toLowerCase() : 'pending';
      if (['pending', 'processing', 'completed', 'failed', 'skipped'].indexOf(queueStatus) === -1) {
        queueStatus = 'pending';
      }
      var item = createElement(
        'div',
        'documents-private-ocr-item documents-private-ocr-item--' + queueStatus
      );
      var copy = createElement('div', 'documents-private-ocr-item__copy');
      var meta = createElement('div', 'documents-private-ocr-item__meta');
      var statuses = createElement('div', 'documents-private-ocr-item__statuses');
      var actions = createElement('div', 'documents-private-ocr-item__actions');
      var ocrBadge = createElement('span', 'documents-private-ocr-status');
      var s3Badge = createElement('span', 'documents-private-ocr-status');
      var isLiveUpload = file.origin === 'live_upload';
      var originBadge = createElement(
        'span',
        'documents-private-ocr-origin documents-private-ocr-origin--' + (isLiveUpload ? 'new' : 'old'),
        isLiveUpload ? 'Новый файл' : 'Старый файл'
      );
      var progressRow = createElement('div', 'documents-private-ocr-item__progress');
      var progressTrack = createElement('div', 'documents-private-ocr-item__progress-track');
      var progressBar = createElement('div', 'documents-private-ocr-item__progress-bar');
      var progressLabelText = Math.round(file.progress) + '%';
      if (file.totalPages > 0) {
        progressLabelText += ' · стр. ' + Math.min(file.currentPage, file.totalPages) + '/' + file.totalPages;
      } else if (queueStatus === 'pending') {
        progressLabelText = 'Ожидает запуска';
      }
      var progressLabel = createElement(
        'div',
        'documents-private-ocr-item__progress-label',
        progressLabelText
      );
      progressBar.style.width = Math.max(0, Math.min(100, file.progress)) + '%';
      progressTrack.appendChild(progressBar);
      progressRow.appendChild(progressTrack);
      progressRow.appendChild(progressLabel);
      copy.appendChild(createElement('div', 'documents-private-ocr-item__name', file.name));
      meta.appendChild(originBadge);
      meta.appendChild(createElement('span', '', 'Задача ' + file.taskLabel));
      meta.appendChild(createElement('span', '', file.fullReprocess
        ? (queueStatus === 'completed'
          ? 'OCR перезаписан у ' + file.participantsTotal + ' участников'
          : 'Новый OCR перезапишет текст у ' + file.participantsTotal + ' участников')
        : 'OCR есть у ' + file.presentCount + ' из ' + file.participantsTotal + ' участников'));
      if (file.size) {
        meta.appendChild(createElement('span', '', formatFileSize(file.size)));
      }
      copy.appendChild(meta);
      statuses.appendChild(ocrBadge);
      statuses.appendChild(s3Badge);
      updateFileStatusBadges(file, ocrBadge, s3Badge);
      ...
      var reasonText = '';
      if (fileState.rawStatus === 'failed'
        || fileState.rawStatus === 'error'
        || fileState.rawStatus === 'unsupported'
      ) {
        reasonText = file.error || 'Сервер не сохранил описание ошибки OCR.';
      } else if (fileState.snapshotStatus === 'error' || fileState.snapshotStatus === 'partial') {
        reasonText = file.snapshotError || 'Не удалось обновить один или несколько пользовательских JSON в S3.';
      }

      if (fileState.completed && file.documentId && file.storedName) {
            var textPanel = createElement('div', 'documents-private-ocr-text');
            var viewButton = createElement('button', 'documents-private-ocr-view', 'Показать OCR-текст');
            var textLoaded = false;
            textPanel.hidden = true;
            textPanel.setAttribute('role', 'region');
            viewButton.type = 'button';
            viewButton.setAttribute('aria-expanded', 'false');
            viewButton.addEventListener('click', function() {
              if (textLoaded) {
                textPanel.hidden = !textPanel.hidden;
                viewButton.setAttribute('aria-expanded', textPanel.hidden ? 'false' : 'true');
                viewButton.textContent = textPanel.hidden ? 'Показать OCR-текст' : 'Скрыть OCR-текст';
                return;
              }
              ...
              viewButton.textContent = 'Загружаю…';
              var resultPayload = {
                action: 'admin_ocr_result',
                organization: state.organization,
                documentId: file.documentId,
                storedName: file.storedName
              };
              mergeTelegramUserId(resultPayload);

              fetch(buildApiUrl('admin_ocr_result'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
                cache: 'no-store',
                ...
                .then(handleResponse)
                .then(function(data) {
                  var sourceLocation = data && data.sourceLocation ? String(data.sourceLocation) : 'Источник не указан';
                  var ocrText = data && typeof data.text === 'string' && data.text
                    ? data.text
                    : 'OCR-сервис вернул пустой текст.';
                  textPanel.textContent = 'Источник текста: ' + sourceLocation + '\n\n' + ocrText;
                  textPanel.hidden = false;
                  textLoaded = true;
                  viewButton.disabled = false;
                  viewButton.textContent = 'Скрыть OCR-текст';
                  viewButton.setAttribute('aria-expanded', 'true');
                })
                .catch(function(error) {
                  viewButton.disabled = false;
                  viewButton.textContent = 'Показать OCR-текст';
                  showMessage('error', error && error.message ? error.message : 'Не удалось получить OCR-текст.');
                });
            });
            actions.appendChild(viewButton);

            var checkPanel = createElement('div', 'documents-private-ocr-check-result');
            var checkButton = createElement(
              'button',
              'documents-private-ocr-check',
              'Проверить telegramId.json'
            );
            checkPanel.hidden = true;
            checkPanel.setAttribute('role', 'status');
            ...
            checkButton.addEventListener('click', function() {
              ...
              var checkPayload = {
                action: 'admin_ocr_s3_check',
                organization: state.organization,
                documentId: file.documentId,
                storedName: file.storedName
              };
              mergeTelegramUserId(checkPayload);

              fetch(buildApiUrl('admin_ocr_s3_check'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
                cache: 'no-store',
                ...
                .then(handleResponse)
                .then(function(data) {
                  var participants = Array.isArray(data && data.participants) ? data.participants : [];
                  var lines = [
                    data && data.message ? String(data.message) : 'Проверка завершена.',
                    'Найдено: ' + (Number(data && data.presentCount) || 0)
                      + ' из ' + (Number(data && data.participantsTotal) || 0)
                  ];
                  participants.forEach(function(participant) {
                    var telegramId = participant && participant.telegramId
                      ? String(participant.telegramId)
                      : 'неизвестный ID';
                    var marker = participant && participant.present ? '✓' : '✕';
                    var detail = participant && participant.present
                      ? (Number(participant.textLength) || 0) + ' симв.'
                      : String(participant && participant.message ? participant.message : 'OCR-текст отсутствует.');
                    var location = participant && participant.sourceLocation
                      ? ' · ' + String(participant.sourceLocation)
                      : '';
                    lines.push(marker + ' ' + telegramId + ' — ' + detail + location);
                  });
                  checkPanel.textContent = lines.join('\n');
                  checkPanel.hidden = false;
                  checkPanel.classList.toggle(
                    'documents-private-ocr-check-result--missing',
                    !(data && data.allPresent)
                  );
                  checkButton.disabled = false;
                  checkButton.textContent = data && data.repairQueued
                    ? 'Проверить восстановление'
                    : 'Проверить снова';

                  file.snapshotStatus = data && data.snapshotStatus ? String(data.snapshotStatus) : '';
                  file.snapshotUsersCount = Number(data && data.presentCount) || 0;
                  file.snapshotDeliveryVersion = data && data.allPresent
                    ? OCR_SNAPSHOT_DELIVERY_VERSION
                    : 0;
                  if (file.fileRef && file.fileRef.ocr && typeof file.fileRef.ocr === 'object') {
                    file.fileRef.ocr.snapshotStatus = file.snapshotStatus;
                    file.fileRef.ocr.snapshotUsersCount = file.snapshotUsersCount;
                    if (data && data.allPresent) {
                      file.fileRef.ocr.snapshotDeliveryVersion = OCR_SNAPSHOT_DELIVERY_VERSION;
                    } else {
                      delete file.fileRef.ocr.snapshotDeliveryVersion;
                    }
                  }
                  updateFileStatusBadges(file, ocrBadge, s3Badge);
                  updateDashboardStats();
                  renderOcrBackfill(backfillState);
                  showMessage(data && (data.allPresent || data.repairQueued) ? 'success' : 'error', lines[0]);
                })
                .catch(function(error) {
                  checkButton.disabled = false;
                  checkButton.textContent = 'Проверить telegramId.json';
                  showMessage('error', error && error.message ? error.message : 'Не удалось проверить S3 JSON.');
                });
            });
            actions.appendChild(checkButton);
            item.appendChild(textPanel);
            item.appendChild(checkPanel);
      }

      var canRetry = file.documentId
        && file.storedName
        && fileState.rawStatus !== 'processing'
        && fileState.rawStatus !== 'pending'
        && fileState.rawStatus !== 'unsupported'
        && fileState.attention;
      if (canRetry) {
            var retryButton = createElement(
              'button',
              'documents-private-ocr-retry',
              fileState.completed ? 'Сохранить в S3' : 'Запустить OCR'
            );
            ...
            retryButton.addEventListener('click', function() {
              ...
              retryButton.textContent = 'Запускаю…';

              var payload = {
                action: 'admin_ocr_retry',
                organization: state.organization,
                documentId: file.documentId,
                storedName: file.storedName
              };
              ...
              fetch(buildApiUrl('admin_ocr_retry'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                credentials: 'same-origin',
                cache: 'no-store',
                ...
                .then(handleResponse)
                .then(function(data) {
                  if (file.fileRef && data && data.file && typeof data.file === 'object') {
                    Object.keys(file.fileRef).forEach(function(key) {
                      delete file.fileRef[key];
                    });
                    Object.assign(file.fileRef, data.file);
                  }
                  var nextStatus = data && data.file && data.file.ocr && data.file.ocr.status
                    ? String(data.file.ocr.status).toLowerCase()
                    : '';
                  showMessage(
                    data && data.queued === true
                      ? 'success'
                      : (nextStatus === 'completed' ? 'success' : 'error'),
                    data && data.message ? data.message : 'Повторный OCR завершён.'
                  );
                  if (modal.isConnected) {
                    closeOcrMonitor();
                    openAdminOcrMonitorModal();
                  }
                })
                .catch(function(error) {
                  retryButton.disabled = false;
                  retryButton.textContent = fileState.completed ? 'Сохранить в S3' : 'Запустить OCR';
                  showMessage('error', error && error.message ? error.message : 'Не удалось повторить OCR.');
                });
            });
      ...
      if (reasonText) {
        var reasonPanel = createElement('div', 'documents-private-ocr-reason', reasonText);
        var reasonButton = createElement('button', 'documents-private-ocr-reason-button', 'Показать ошибку');
        reasonPanel.hidden = true;
        reasonPanel.setAttribute('role', 'status');
        reasonButton.type = 'button';
        reasonButton.setAttribute('aria-expanded', 'false');
        reasonButton.addEventListener('click', function() {
          var shouldOpen = reasonPanel.hidden;
          reasonPanel.hidden = !shouldOpen;
          reasonButton.setAttribute('aria-expanded', shouldOpen ? 'true' : 'false');
          reasonButton.textContent = shouldOpen ? 'Скрыть ошибку' : 'Показать ошибку';
        });
        actions.appendChild(reasonButton);
        item.appendChild(reasonPanel);
      }
    ...
    function renderFileList() {
      ...
      if (!allFiles.length) {
        list.appendChild(createElement(
          'div',
          'documents-responses-empty',
          searchQuery ? 'По этому запросу файлов нет.' : 'В этом разделе файлов нет.'
        ));
      }
      visibleCount.textContent = ocrItemsTotal > 0
        ? 'Показано ' + (ocrItemsOffset + 1) + '–' + Math.min(ocrItemsTotal, ocrItemsOffset + allFiles.length)
          + ' из ' + ocrItemsTotal
        : 'Файлов нет';
    ...
    addSummaryCard('total', 'Всего файлов в очереди');
    addSummaryCard('processing', 'В очереди или работе');
    addSummaryCard('completed', 'OCR-текст получен');
    addSummaryCard('attention', 'Требуют внимания', 'attention');
    addSummaryCard('synced', 'Сохранено всем в S3', 'success');

    searchInput.type = 'search';
    searchInput.placeholder = 'Поиск по файлу или номеру задачи';
    searchInput.setAttribute('aria-label', 'Поиск файлов OCR');
    ...
    filters.setAttribute('aria-label', 'Фильтр файлов OCR');
    [
      { key: 'all', label: 'Все' },
      { key: 'attention', label: 'Требуют внимания' },
      { key: 'processing', label: 'В работе' },
      { key: 'ready', label: 'Готовы в S3' },
      { key: 'new', label: 'Новые' },
      { key: 'old', label: 'Старые' }
    ].forEach(function(filter) {
      var button = createElement(
        'button',
        'documents-private-ocr-filter' + (filter.key === activeFilter ? ' is-active' : ''),
        filter.label
      );
      button.type = 'button';
      button.dataset.filter = filter.key;
      button.setAttribute('aria-pressed', filter.key === activeFilter ? 'true' : 'false');
      filters.appendChild(button);
    });
    filters.addEventListener('click', function(event) {
      var button = event.target.closest('.documents-private-ocr-filter');
      if (!button || !filters.contains(button)) {
        return;
      }
      activeFilter = button.dataset.filter || 'all';
      filters.querySelectorAll('.documents-private-ocr-filter').forEach(function(filterButton) {
    ...
    ocrPreviousButton.type = 'button';
    ocrNextButton.type = 'button';
    ocrPreviousButton.addEventListener('click', function() {
      ocrItemsOffset = Math.max(0, ocrItemsOffset - OCR_ADMIN_PAGE_SIZE);
      list.scrollTop = 0;
      updateOcrBackfill('status', false);
    });
    ocrNextButton.addEventListener('click', function() {
      ocrItemsOffset += OCR_ADMIN_PAGE_SIZE;
      list.scrollTop = 0;
      updateOcrBackfill('status', false);
    });
    ocrPager.appendChild(ocrPreviousButton);
    ocrPager.appendChild(ocrPageLabel);
    ocrPager.appendChild(ocrNextButton);
    toolbar.appendChild(searchInput);
    toolbar.appendChild(filters);
    toolbar.appendChild(ocrPager);
    heading.appendChild(createElement('div', 'documents-private-ocr-group__title', 'Последовательная очередь OCR: новые и старые файлы'));
    heading.appendChild(visibleCount);
    visibleCount.setAttribute('aria-live', 'polite');
    list.tabIndex = 0;
    list.setAttribute('aria-label', 'Список OCR-файлов. Используйте колесо мыши или клавиши со стрелками для прокрутки.');
    ...
    viewTabs.setAttribute('aria-label', 'Раздел OCR-монитора');
    viewTabs.appendChild(filesTabButton);
    viewTabs.appendChild(telegramTabButton);
    panel.appendChild(header);
    panel.appendChild(viewTabs);
    panel.appendChild(backfillPanel);
    panel.appendChild(snapshotAuditPanel);
    panel.appendChild(body);
    modal.appendChild(panel);
    document.body.appendChild(modal);
    document.addEventListener('keydown', handleOcrEscape, true);
    snapshotAuditPanel.hidden = true;
    renderOcrBackfill({});
    renderSnapshotAudit({});
    updateOcrBackfill('status', true);
    backfillTimerId = window.setInterval(pollOcrBackfill, OCR_BACKFILL_POLL_INTERVAL_MS);
  }
...
function ensureAdminTemplateStyles() {
    ...
    style.textContent = '' +
      ...
      '.documents-admin__ocr-button{margin-right:8px;transition:border-color .16s ease,background .16s ease,color .16s ease}.documents-admin__ocr-button.is-running{border-color:#86efac;background:#ecfdf5;color:#047857}.documents-admin__ocr-button.is-starting{border-color:#93c5fd;background:#eff6ff;color:#1d4ed8}.documents-admin__ocr-button.is-stopped{border-color:#fecaca;background:#fff7f7;color:#b91c1c}' +
