...
switch ($action) {
    ...
    case 'mini_app_tasks':
        ...
        $botTokenForDue = docs_resolve_telegram_bot_token();
        if ($telegramUserId !== '' && $botTokenForDue !== null && $botTokenForDue !== '') {
            ...
            foreach ($tasks as $taskRecord) {
                ...
                $statusKey = docs_status_key_from_status((string) ($taskRecord['status'] ?? ''));
                if ($statusKey === 'done' || $statusKey === 'cancelled') {
                    continue;
                }
                $taskDue = sanitize_date_field((string) ($taskRecord['dueDate'] ?? ''));
                if ($taskDue === '') {
                    continue;
                }
                $dueTs = strtotime($taskDue);
                if ($dueTs === false) {
                    continue;
                }
                ...
                $summary = sanitize_text_field((string) ($taskRecord['summary'] ?? ''), 220);
                $executor = sanitize_text_field((string) ($taskRecord['executor'] ?? ''), 180);
                $message = $notifyType === 'day_before'
                    ? "⏰ Напоминание: завтра срок задачи.\nФИО: " . ($executor !== '' ? $executor : 'не указан')
                        . "\n№ задачи: " . $taskId
