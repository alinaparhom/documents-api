  ...
  async function resolvePaidSourceFiles(state, config) {
    ...
    for (var i = 0; i < candidates.length; i += 1) {
      ...
      try {
        ...
        if (file && file.fileObject) {
          var fallbackName = file.name || 'document.bin';
          if (isPdfFile(fallbackName, file.fileObject.type) && !/\.pdf$/i.test(fallbackName)) fallbackName += '.pdf';
          preparedFiles.push({ name: fallbackName, blob: file.fileObject });
        }
        continue;
      }
    }
