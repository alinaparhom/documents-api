(function () {
  ...
  var DEFAULT_AI_BEHAVIOR = 'ТЫ — ИИ ДЛЯ РЕЖИМА «ОТВЕТ С ПОМОЩЬЮ ИИ».\n'    + '\n'    + 'ЗАДАЧА: ДАТЬ ТОЛЬКО ГОТОВЫЙ ТЕКСТ, КОТОРЫЙ СРАЗУ ВСТАВЛЯЕТСЯ В ДОКУМЕНТ.\n'    + '\n'    + '=== ОБЯЗАТЕЛЬНО ===\n'    + '1. ПИШИ ТОЛЬКО ПО СУЩЕСТВУ ДОКУМЕНТА: ФАКТЫ, ВЫВОДЫ, ДЕЙСТВИЯ.\n'    + '2. НЕ ДОБАВЛЯЙ ПРИВЕТСТВИЯ, ОБРАЩЕНИЯ, ПРОЩАНИЯ, ПОДПИСИ.\n'    + '3. НЕ ДОБАВЛЯЙ ИМЕНА, ДОЛЖНОСТИ, РЕКВИЗИТЫ, НОМЕРА СЧЕТОВ, КОНТАКТЫ, ДАТУ ПИСЬМА.\n'    + '4. НЕ ПИШИ МЕТАКОММЕНТАРИИ: «АНАЛИЗ», «РАЗБОР», «Я ПРОАНАЛИЗИРОВАЛ».\n'    + '5. НЕ ВЫДУМЫВАЙ ДАННЫЕ, КОТОРЫХ НЕТ В ФАЙЛЕ.\n'    + '6. ЕСЛИ ДАННЫХ НЕДОСТАТОЧНО — КРАТКО УКАЖИ, ЧЕГО ИМЕННО НЕ ХВАТАЕТ, БЕЗ СЛУЖЕБНЫХ ФРАЗ.\n'    + '\n'    + '=== ФОРМАТ ВЫВОДА ===\n'    + '- Возвращай только основной текст ответа для вставки в документ.\n'    + '- Без шапки письма и без служебного оформления.\n';
  ...
  function openDocumentsAiBriefSummaryModal(config) {
    ...
    function setPreviewText(text, mode) {
      if (mode === 'plain') {
      ...
      preview.innerHTML = renderPrettyBrief(safeText);
    }
    ...
    sources.forEach(function(source) {
      ...
      button.addEventListener('click', function() {
        ...
        setPreviewText('⏳ Обрабатываю файл...', 'plain');
        var startedAt = Date.now();
        requestBriefVisionByFile(source, function(message) { setPreviewText(message || '⏳ Обрабатываю файл...', 'plain'); })
          ...
          .catch(function(error) {
          setPreviewText('Ошибка: ' + (error && error.message ? error.message : 'неизвестная ошибка'), 'plain');
