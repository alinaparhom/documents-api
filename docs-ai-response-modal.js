(function() {
  ...
  function ensureStyle() {
    ...
    style.textContent = '' +
      ...
      '@media (max-width:768px){.documents-ai-modal{padding:6px;align-items:flex-end;}.documents-ai-modal__panel{width:100%;max-height:95vh;border-radius:18px;padding:10px;}.documents-ai-modal__grid{grid-template-columns:1fr;}.documents-ai-modal__button{flex:1 1 calc(50% - 8px);}.documents-ai-modal__textarea{min-height:100px;font-size:16px;}.documents-ai-modal__textarea--response{min-height:220px;}}';
  ...
  function openDocumentsAiResponseModal(options) {
    ...
    var desc = createElement('div', 'documents-ai-modal__desc', 'Только файл + ваш текст. Справа минималистичный чат с ИИ.');
    ...
    var remoteFilesHint = createElement('div', 'documents-ai-modal__hint', linkedRemoteFiles.length
      ? ('Подгруженные файлы: ' + linkedRemoteFiles.map(function(file) { return file.name; }).join(', '))
      : 'Файлы из строки задачи не найдены.');

    var promptLabel = createElement('div', 'documents-ai-modal__label', 'Ваш запрос к ИИ');
    var promptInput = createElement('textarea', 'documents-ai-modal__textarea');
    promptInput.placeholder = 'Например: Подготовь официальный ответ на это письмо.';

    var responseLabel = createElement('div', 'documents-ai-modal__label', 'Текст ответа (можно править перед PDF)');
    ...
    var status = createElement('div', 'documents-ai-modal__status', 'Нажмите «Отправить в ИИ».');
    ...
    var analyzeButton = createElement('button', 'documents-ai-modal__button documents-ai-modal__button--primary', 'Отправить в ИИ');
    ...
    analyzeButton.addEventListener('click', function() {
      ...
      showStatus('Идёт обработка файла и генерация ответа…', false);
      ...
      fetchRemoteFiles(linkedRemoteFiles)
        ...
        .then(function(data) {
          ...
          var local = buildDraftPair(promptInput.value, titleInput.value, '');

          responseInput.value = serverResponse || local.neutral;
          ...
          appendChatMessage('assistant', responseInput.value);
          showStatus('Готово: ответ получен.', false);
        })
        .catch(function(error) {
          var fallback = buildDraftPair(promptInput.value, titleInput.value, '');
          responseInput.value = fallback.neutral;
          citationsBlock.textContent = '';
    ...
    leftCard.appendChild(responseLabel);
    leftCard.appendChild(responseInput);
