(function () {
  ...
  function openDocumentsAiResponseModal(options) {
    ...
    function clearRetryCountdown() {
      ...
      state.retryUntilMs = 0;
      state.retryMessageNode = null;
    }
    function getRetrySecondsLeft() {
      if (!state.retryUntilMs) {
        return 0;
      }
      return Math.max(0, Math.ceil((state.retryUntilMs - Date.now()) / 1000));
    }
    function showRetryCountdown(seconds) {
      var remaining = Math.max(1, Number(seconds || 1));
      var countdownMessage = state.retryMessageNode;
      if (!countdownMessage) {
        countdownMessage = createMessage('assistant', 'До бесплатной попытки осталось: ' + remaining + ' сек.', true);
        messages.appendChild(countdownMessage);
        state.retryMessageNode = countdownMessage;
      } else {
        countdownMessage.innerHTML = escapeHtml('До бесплатной попытки осталось: ' + remaining + ' сек.');
      }
      messages.scrollTop = messages.scrollHeight;
      clearRetryCountdown();
      state.retryUntilMs = Date.now() + (remaining * 1000);
      state.retryMessageNode = countdownMessage;
      retryCountdownTimer = setInterval(function () {
        remaining = getRetrySecondsLeft();
        if (remaining <= 0) {
          clearRetryCountdown();
    ...
    async function sendMessage() {
      ...
      var retryLeft = getRetrySecondsLeft();
      if (retryLeft > 0) {
        showRetryCountdown(retryLeft);
        return;
      }
