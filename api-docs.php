...
function extractParsedTextFromOcrJson(array $ocrJson): string
{
...
function scoreOcrTextQuality(string $text): float
{
    $text = trim($text);
    if ($text === '') {
        return 0.0;
    ...
    $bad = preg_match_all('/[�]/u', $text) ?: 0;
    $ratio = $length > 0 ? ($letters / $length) : 0.0;
    return ($length * 0.6) + ($ratio * 100.0) - ($bad * 10.0);
}
...
if ($action === 'ocr_extract') {
    ...
    foreach ($preparedFiles as $index => $preparedFile) {
        ...
        $usedEngine = (string)$ocrExtraFields['OCREngine'];
        ...
        $primaryText = extractParsedTextFromOcrJson($ocrJson);
        $primaryScore = scoreOcrTextQuality($primaryText);
        $bestOcrJson = $ocrJson;
        $bestScore = $primaryScore;
        $alternateEngine = $usedEngine === '2' ? '1' : '2';
        if ($ocrPreprocessEnabled && $primaryScore < 120.0) {
            $fallbackAttempted = true;
            $fallbackFields = $ocrExtraFields;
            $fallbackFields['OCREngine'] = $alternateEngine;
            $fallbackStartedAt = microtime(true);
            $fallbackResult = performOcrRequest(
                $ocrBaseUrl,
                $ocrApiKey,
                $preparedFile,
                $targetLanguage,
            ...
            $fallbackStatus = (int)$fallbackResult['status'];
            $fallbackDurationMs = (int)round((microtime(true) - $fallbackStartedAt) * 1000);
            if ($fallbackBody !== false && $fallbackStatus < 400) {
                $fallbackJson = json_decode((string)$fallbackBody, true);
                if (is_array($fallbackJson)) {
                    $fallbackText = extractParsedTextFromOcrJson($fallbackJson);
                    $fallbackScore = scoreOcrTextQuality($fallbackText);
                    if ($fallbackScore > $bestScore) {
                        $bestScore = $fallbackScore;
                        $bestOcrJson = $fallbackJson;
                        $usedEngine = $alternateEngine;
        ...
        $parsedResults = isset($bestOcrJson['ParsedResults']) && is_array($bestOcrJson['ParsedResults']) ? $bestOcrJson['ParsedResults'] : [];
    ...
    jsonResponse(200, [
        ...
        'raw' => [
            ...
            'ocrRequest' => [
                ...
                'maxQualityMode' => $ocrPreprocessEnabled,
...
$styleExampleInstruction = "Ориентируйся на форму ответа: «В ответ на Ваш документ от 20.03.2026 № 15/02 сообщаем следующее... Отмечаем, что... Обращаем Ваше внимание на...»";
...
$systemMessage = "Ты помощник по деловой переписке на русском языке. Верни только JSON объект с полями: analysis, decision, decision_reason, risks, required_actions, response. "
  ...
  . "В response пиши развёрнутый официальный ответ (не краткий и не короткий), в роли руководителя или иного уполномоченного лица, принимающего окончательное решение. "
  ...
  . "Соблюдай структуру: вступление, основная часть, заключение. "
  . "Не пересказывай OCR-текст дословно и не переписывай весь документ. Определи, что требуется в документе, и прими решение по сути требований. "
  . "При достаточности данных выбирай только approve или reject; need_clarification используй только если ключевых данных объективно не хватает. "
  . $styleInstruction . ' '
  ...
  . 'Если данных недостаточно, запроси недостающие сведения и укажи, что после их получения будет принято решение need_clarification. '
