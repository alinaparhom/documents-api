...
function normalizeDecisionValue(string $value): string
{
    ...
    if (in_array($normalized, ['need_revision', 'revision', 'rework', 'return_for_revision', 'доработка', 'на доработку', 'вернуть на доработку'], true)) {
        return 'need_revision';
    }
    if (in_array($normalized, ['acknowledge', 'noted', 'for_information', 'take_note', 'принять к сведению', 'к сведению'], true)) {
        return 'acknowledge';
    }
...
$systemMessage = "Ты помощник по деловой переписке на русском языке. Верни только JSON объект с полями: analysis, decision, decision_reason, risks, required_actions, response. "
  ...
  . $behaviorInstruction . ' Решение decision должно быть строго одним из: approve, reject, need_revision, acknowledge, need_clarification. Поля risks и required_actions верни массивами строк. '
...
if (!$decisionBlock['valid']) {
    ...
    $repairBody['messages'][] = [
        'role' => 'user',
        'content' => 'Верни только корректный JSON без пояснений. Обязательные поля: analysis, decision (approve|reject|need_revision|acknowledge|need_clarification), decision_reason, risks[], required_actions[], response.'
    ];
