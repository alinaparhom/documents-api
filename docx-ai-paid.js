...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function buildTemplateDocxPreviewHtml() {
    var templateUrl = '';
    try {
    ...
    return fetch(templateUrl, { credentials: 'same-origin', cache: 'no-store' })
      ...
      .then(function(result) {
        return String(result && result.value || '').trim() || '<p>Шаблон пуст.</p>';
      });
  }
  ...
  function openTemplatePreviewModal(html) {
    ...
    overlay.innerHTML = '<div class="documents-vip-template-preview__card"><div class="documents-vip-template-preview__head"><div class="documents-vip-template-preview__title">Просмотр template.docx</div><button type="button" class="documents-vip-template-preview__close" data-close aria-label="Закрыть">×</button></div><div class="documents-vip-template-preview__body">' + String(html || '') + '</div></div>';
