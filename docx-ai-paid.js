...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function openDocxRenderPreviewLocal(blob, fileName) {
    ...
    overlay.innerHTML = '<div class="docx-template-preview__card"><div class="docx-template-preview__head"><div><div class="docx-template-preview__title">Локальный предпросмотр</div><div class="docx-template-preview__hint">Office-стиль: страницы, поля и разрывы</div></div><div class="docx-template-preview__actions"><button type="button" class="docx-template-preview__btn docx-template-preview__btn--primary" data-preview-download>Скачать</button><button type="button" class="docx-template-preview__btn" data-preview-close>Закрыть</button></div></div><div class="docx-template-preview__body"><div class="docx-template-preview__doc" data-preview-doc aria-label="DOCX preview"></div><div class="docx-template-preview__loading" data-preview-loading><div class="docx-template-preview__loading-card"><div class="docx-template-preview__loading-title">Готовим локальный предпросмотр…</div><div class="docx-template-preview__loading-sub">Это может занять несколько секунд на телефоне.</div><div class="docx-template-preview__bar"></div></div></div></div><div class="docx-template-preview__status" data-preview-status>Подготовка локального предпросмотра…</div></div>';
    document.body.appendChild(overlay);
    ...
    ensureDocxPreviewLibrariesLoaded().then(function(renderer) {
      ...
      statusNode.textContent = 'Готово: локальный Office-предпросмотр открыт (с разбивкой на страницы).';
    }).catch(function(error) {
      if (loadingNode) loadingNode.style.display = 'none';
