...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function openDocumentsVipAiPaidModal(config) {
    ...
    var templateOpenButton = panel.querySelector('[data-template-open]');
    ...
    if (templateOpenButton) {
      templateOpenButton.addEventListener('click', function() {
        openTemplateAnswerEditor(templateOpenButton);
      });
    }
