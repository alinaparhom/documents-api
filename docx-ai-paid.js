...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function openDocxRenderPreviewPage(blob) {
    ...
    blobToDataUrl(blob).then(function(dataUrl) {
      frame.srcdoc = '<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><style>:root{--a4-width:210mm;--a4-height:297mm}*{box-sizing:border-box}html,body{margin:0;height:100%}body{background:#e2e8f0;font-family:Inter,system-ui,sans-serif;overflow:auto}#preview{height:100%;padding:12px;overflow:auto;display:flex;justify-content:center;align-items:flex-start}#docx{width:var(--a4-width);min-width:var(--a4-width);max-width:var(--a4-width)}#docx .docx-wrapper{background:transparent;border:none;box-shadow:none;padding:0;min-height:auto;overflow:visible}#docx .docx{width:var(--a4-width)!important;max-width:var(--a4-width)!important;min-height:var(--a4-height);margin:0 0 14px 0;background:#fff;border:1px solid rgba(203,213,225,.9);box-shadow:0 12px 28px rgba(15,23,42,.12)}.err{margin:12px;background:#fee2e2;color:#991b1b;border:1px solid #fecaca;border-radius:10px;padding:10px;font-size:13px}</style><script src=\"https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js\"><\/script><script src=\"https://cdn.jsdelivr.net/npm/docx-preview@0.3.6/dist/docx-preview.min.js\"><\/script></head><body><div id=\"preview\"><div id=\"docx\"></div></div><div id=\"error\" class=\"err\" style=\"display:none\"></div><script>(async function(){try{var dataUrl=' + JSON.stringify(dataUrl) + ';var response=await fetch(dataUrl);var arrayBuffer=await response.arrayBuffer();var container=document.getElementById(\"docx\");var renderer=(window.docx&&window.docx.renderAsync)?window.docx:(window.docxPreview&&window.docxPreview.renderAsync?window.docxPreview:null);if(!renderer||typeof renderer.renderAsync!==\"function\"){throw new Error(\"docx_preview_not_loaded\");}await renderer.renderAsync(arrayBuffer,container,null,{inWrapper:true,breakPages:true,ignoreWidth:false,ignoreHeight:false});}catch(e){var err=document.getElementById(\"error\");err.style.display=\"block\";err.textContent=\"Не удалось отрендерить DOCX: \"+(e&&e.message?e.message:\"unknown\");}})();<\/script></body></html>';
