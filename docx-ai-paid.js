...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function replaceMarkerInContainer(container, markerText, replacementText) {
    ...
    function buildReplacementFragment(textValue) {
      var fragment = document.createDocumentFragment();
      var normalized = String(textValue || '').replace(/\r\n/g, '\n').replace(/\r/g, '\n');
    ...
    for (var m = matches.length - 1; m >= 0; m -= 1) {
      ...
      range.insertNode(buildReplacementFragment(replacementText));
      range.detach();
    }
  ...
  function openDocxRenderPreviewPage(blob) {
    ...
    blobToDataUrl(blob).then(function(dataUrl) {
      var html = '<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1, viewport-fit=cover"><title>Превью DOCX</title><style>:root{--bg:#eef4ff;--panel:#ffffffd9;--text:#0f172a;--muted:#64748b;--line:#cbd5e1;--accent:#2563eb}*{box-sizing:border-box}body{margin:0;font-family:Inter,system-ui,-apple-system,BlinkMacSystemFont,\"Segoe UI\",sans-serif;background:radial-gradient(circle at top,#f8fbff,#eef4ff 45%,#e2e8f0);color:var(--text)}.shell{min-height:100dvh;display:flex;flex-direction:column}.top{position:sticky;top:0;z-index:10;display:flex;flex-wrap:wrap;align-items:center;gap:8px;padding:10px calc(12px + env(safe-area-inset-right)) calc(10px + env(safe-area-inset-bottom,0px)) calc(12px + env(safe-area-inset-left));background:var(--panel);backdrop-filter:blur(14px);border-bottom:1px solid rgba(148,163,184,.32)}.btn{border:1px solid var(--line);background:#fff;border-radius:12px;padding:10px 12px;font-size:13px;font-weight:700;color:#0f172a}.btn.primary{background:linear-gradient(135deg,#dbeafe,#eff6ff);color:#1d4ed8;border-color:#bfdbfe}.group{display:inline-flex;align-items:center;gap:6px;padding:6px 8px;border-radius:12px;background:#fff;border:1px solid rgba(203,213,225,.85)}.group label{font-size:12px;color:var(--muted);font-weight:700}.zoom{accent-color:var(--accent);width:120px}.status{font-size:12px;color:var(--muted);margin-left:auto}.canvas{flex:1;overflow:auto;padding:12px}.page{max-width:1120px;margin:0 auto;transform-origin:top center}.docx-wrapper{background:#fff;border:1px solid rgba(203,213,225,.9);border-radius:16px;box-shadow:0 18px 40px rgba(15,23,42,.12);padding:14px;min-height:160px}.loader{display:flex;align-items:center;gap:10px;padding:10px 12px;border:1px dashed rgba(148,163,184,.55);border-radius:12px;color:#334155;background:rgba(255,255,255,.75)}.dot{width:9px;height:9px;border-radius:999px;background:#3b82f6;animation:pulse 1s infinite ease-in-out}@keyframes pulse{0%,100%{opacity:.25;transform:scale(.8)}50%{opacity:1;transform:scale(1)}}.err{margin:10px auto 0;max-width:1120px;background:#fee2e2;color:#991b1b;border:1px solid #fecaca;border-radius:12px;padding:10px 12px;font-size:13px;display:none}@media (max-width:760px){.top{gap:6px;padding:8px calc(10px + env(safe-area-inset-right)) calc(8px + env(safe-area-inset-bottom,0px)) calc(10px + env(safe-area-inset-left))}.btn{flex:1 1 auto;min-height:40px}.group{width:100%;justify-content:space-between}.zoom{width:52vw}.status{width:100%;margin-left:0}.canvas{padding:8px}.docx-wrapper{padding:10px;border-radius:12px}}</style><script src=\"https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js\"><\/script><script src=\"https://cdn.jsdelivr.net/npm/docx-preview@0.3.6/dist/docx-preview.min.js\"><\/script></head><body><div class=\"shell\"><div class=\"top\"><button id=\"download\" class=\"btn primary\">Скачать DOCX</button><button id=\"close\" class=\"btn\">Закрыть</button><div class=\"group\"><label for=\"zoom\">Масштаб</label><input id=\"zoom\" class=\"zoom\" type=\"range\" min=\"55\" max=\"160\" value=\"100\" step=\"5\"><strong id=\"zoom-value\">100%</strong></div><div id=\"status\" class=\"status\">Готовлю предпросмотр…</div></div><div class=\"canvas\"><div class=\"page\" id=\"page\"><div class=\"docx-wrapper\" id=\"docx\"><div class=\"loader\"><span class=\"dot\"></span><span>Рендерим документ через docx-preview…</span></div></div></div><div id=\"error\" class=\"err\"></div></div></div><script>(async function(){var dataUrl=' + JSON.stringify(dataUrl) + ';var status=document.getElementById(\"status\");var err=document.getElementById(\"error\");var zoom=document.getElementById(\"zoom\");var zoomValue=document.getElementById(\"zoom-value\");var page=document.getElementById(\"page\");var container=document.getElementById(\"docx\");var applyZoom=function(v){var value=Math.max(55,Math.min(160,Number(v)||100));page.style.transform=\"scale(\"+(value/100)+\")\";zoomValue.textContent=value+\"%\";};zoom.addEventListener(\"input\",function(){applyZoom(zoom.value);});applyZoom(zoom.value);try{var response=await fetch(dataUrl);var arrayBuffer=await response.arrayBuffer();var renderer=(window.docx&&window.docx.renderAsync)?window.docx:(window.docxPreview&&window.docxPreview.renderAsync?window.docxPreview:null);if(renderer&&typeof renderer.renderAsync===\"function\"){await renderer.renderAsync(arrayBuffer,container,null,{inWrapper:true,breakPages:true,ignoreWidth:false,ignoreHeight:false,experimental:true,useBase64URL:true});status.textContent=\"Готово\";}else{throw new Error(\"docx_preview_not_loaded\");}}catch(e){err.style.display=\"block\";err.textContent=\"Не удалось отрендерить DOCX: \"+(e&&e.message?e.message:\"unknown\");status.textContent=\"Ошибка\";}document.getElementById(\"download\").addEventListener(\"click\",function(){var a=document.createElement(\"a\");a.href=dataUrl;a.download=\"template-answer.docx\";document.body.appendChild(a);a.click();document.body.removeChild(a);});})();document.getElementById(\"close\").addEventListener(\"click\",function(){window.close();});<\/script></body></html>';
