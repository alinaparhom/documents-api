...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function ensureVipAiModalStyles() {
    ...
    style.textContent += '.documents-vip-ai__composer{display:grid;grid-template-columns:1fr;gap:10px;padding:10px;background:linear-gradient(145deg,rgba(255,255,255,.88),rgba(248,250,252,.84));border:1px solid rgba(191,219,254,.55)}.documents-vip-ai__composer-main{display:grid;gap:8px;min-width:0}.documents-vip-ai__input{width:100%;max-width:100%;min-height:88px;resize:vertical;border:1px solid rgba(191,219,254,.8);border-radius:14px;padding:12px 13px;background:rgba(255,255,255,.96);font-size:14px;line-height:1.48;color:#0f172a;box-shadow:inset 0 1px 2px rgba(15,23,42,.05)}.documents-vip-ai__input::placeholder{color:#94a3b8}.documents-vip-ai__input:focus{outline:none;box-shadow:0 0 0 3px rgba(56,189,248,.25),inset 0 1px 2px rgba(15,23,42,.04);border-color:rgba(56,189,248,.7)}.documents-vip-ai__composer-actions{display:flex;gap:8px;align-items:center;justify-content:space-between;flex-wrap:wrap}.documents-vip-ai__composer-tools{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center;min-width:0}.documents-vip-ai__composer-tools .documents-vip-ai__select{width:100%;min-width:0}.documents-vip-ai__voice-status{font-size:12px;color:#475569;padding:6px 10px;border-radius:999px;background:rgba(239,246,255,.85);border:1px solid rgba(191,219,254,.95)}.documents-vip-ai__action-btn{border:1px solid rgba(148,163,184,.35);background:linear-gradient(135deg,rgba(255,255,255,.98),rgba(248,250,252,.92));color:#0f172a;border-radius:12px;min-height:44px;padding:0 14px;font-size:13px;font-weight:700;box-shadow:0 8px 18px rgba(15,23,42,.08);transition:transform .16s ease,box-shadow .16s ease}.documents-vip-ai__action-btn:hover{transform:translateY(-1px);box-shadow:0 10px 22px rgba(15,23,42,.12)}.documents-vip-ai__action-btn--send{background:linear-gradient(135deg,#2563eb,#0ea5e9);color:#fff;border-color:rgba(37,99,235,.4)}.documents-vip-ai__action-btn--voice.is-active{background:linear-gradient(135deg,rgba(16,185,129,.22),rgba(45,212,191,.2));border-color:rgba(16,185,129,.55);color:#065f46}.documents-vip-ai__action-btn[disabled]{opacity:.55;cursor:not-allowed;box-shadow:none;transform:none}.documents-vip-ai__composer-actions-right{display:flex;gap:8px;align-items:center}@media (max-width:768px){.documents-vip-ai__composer{gap:8px;padding:8px}.documents-vip-ai__composer-tools{grid-template-columns:1fr}.documents-vip-ai__action-btn,.documents-vip-ai__template-btn{min-height:46px;font-size:14px}.documents-vip-ai__input{min-height:102px;font-size:15px}.documents-vip-ai__composer-actions{align-items:stretch;gap:7px}.documents-vip-ai__voice-status{width:100%;text-align:center}.documents-vip-ai__composer-actions-right{width:100%}.documents-vip-ai__composer-actions-right .documents-vip-ai__action-btn{flex:1}.documents-vip-ai__template-btn{width:100%}}';
  ...
  function openDocumentsVipAiPaidModal(config) {
    ...
    var chatInput = panel.querySelector('[data-chat-input]');
    ...
    pushChat('assistant', 'Готово 👌 Выберите стиль ответа, напишите вопрос и нажмите «✈️ Отправить». Для диктовки используйте «🎤 Запись».');
    ...
    var TRANSCRIBE_MODEL = 'whisper-large-v3-turbo';
    var paidEndpoints = ['/js/documents/api-groq-paid.php', '/api-groq-paid.php'];

    function updateVoiceStatus(text, isActive) {
      if (voiceStatusNode) {
        voiceStatusNode.textContent = String(text || '');
      }
      if (voiceButton) {
        voiceButton.classList.toggle('is-active', Boolean(isActive));
        voiceButton.textContent = isActive ? '⏹️ Стоп' : '🎤 Запись';
      }
      isVoiceRecording = Boolean(isActive);
    }
    ...
    function postVoiceTranscription(audioBlob, fileName) {
      function postWithFallback(endpointIndex) {
        if (endpointIndex >= paidEndpoints.length) {
          return Promise.reject(new Error('Сервис распознавания временно недоступен. Попробуйте ещё раз.'));
        }
        var formData = new FormData();
        formData.append('action', 'transcribe_audio');
        formData.append('transcribe_model', TRANSCRIBE_MODEL);
        formData.append('audio', audioBlob, fileName || 'voice-message.webm');
        return fetch(paidEndpoints[endpointIndex], { method: 'POST', body: formData, credentials: 'same-origin' })
          .then(function(response) {
            if ((response.status === 404 || response.status === 405) && endpointIndex < paidEndpoints.length - 1) {
      ...
      return postWithFallback(0).then(function(result) {
        if (!result.response || !result.response.ok || !result.payload || result.payload.ok !== true) {
          throw new Error((result.payload && result.payload.error) || 'Не удалось распознать аудио.');
        }
        var text = String(result.payload.text || '').trim();
        if (!text) {
          throw new Error('Не удалось получить текст из записи.');
        }
        return {
          text: text,
          model: String(result.payload.model || TRANSCRIBE_MODEL)
        };
      });
    }
    ...
    if (voiceButton) {
      var mediaSupport = typeof window !== 'undefined'
        && window.MediaRecorder
        && navigator.mediaDevices
        && typeof navigator.mediaDevices.getUserMedia === 'function';
      if (!mediaSupport) {
        voiceButton.disabled = true;
        updateVoiceStatus('🎙️ Голосовой ввод недоступен в этом браузере.', false);
      } else {
        voiceButton.addEventListener('click', function() {
          ...
          navigator.mediaDevices.getUserMedia({
            ...
            mediaStream = stream;
            var mimeCandidates = ['audio/webm;codecs=opus', 'audio/webm', 'audio/mp4', 'audio/ogg'];
            var selectedMime = '';
            for (var i = 0; i < mimeCandidates.length; i += 1) {
              if (window.MediaRecorder.isTypeSupported(mimeCandidates[i])) {
                selectedMime = mimeCandidates[i];
                break;
              }
            }
            mediaRecorder = selectedMime ? new MediaRecorder(stream, { mimeType: selectedMime }) : new MediaRecorder(stream);
            mediaRecorder.onstart = function() {
              updateVoiceStatus('🔴 Идёт запись. Нажмите «🎤 Запись» ещё раз для остановки.', true);
            };
            ...
            mediaRecorder.onerror = function() {
              updateVoiceStatus('⚠️ Ошибка записи. Проверьте микрофон и попробуйте снова.', false);
              stopVoiceStream();
            };
            mediaRecorder.onstop = function() {
              updateVoiceStatus('⏳ Распознаём речь через Whisper…', false);
              var recordedBlob = new Blob(voiceChunks, { type: (mediaRecorder && mediaRecorder.mimeType) || 'audio/webm' });
              stopVoiceStream();
              voiceChunks = [];
              if (!recordedBlob || !recordedBlob.size) {
                updateVoiceStatus('⚠️ Запись пустая. Скажите фразу и попробуйте ещё раз.', false);
                return;
              }
            ...
            updateVoiceStatus('🚫 Нет доступа к микрофону. Разрешите доступ и повторите.', false);
          });
        });
      }
    }
  ...
  async function openDocxRenderPreviewPage(previewPayload, context) {
    ...
    var previousOverflow = document.body.style.overflow;
    document.body.style.overflow = 'hidden';
    var docNode = overlay.querySelector('[data-preview-doc]');
    ...
    async function showLocalPreview() {
      ...
      try {
        ...
        var renderer = await ensureDocxPreviewLibrariesLoaded();
        setStep(2, 'Подключаем движок предпросмотра...');
        var arrayBuffer = await localBlob.arrayBuffer();
        ...
        if (loadingNode) loadingNode.style.display = 'none';
        statusNode.textContent = 'Готово: открыт локальный предпросмотр.';
      } catch (error) {
        statusNode.textContent = 'Не удалось открыть локальный предпросмотр. Можно открыть через Office Viewer.';
        if (loadingNode) loadingNode.style.display = 'none';
      } finally {
