(function() {
  ...
  var TEMPLATE_DOC_PATH = '/js/documents/app/templates/template.docx';
...
Помни: пользователь обратился к тебе за помощью. Сделай всё, чтобы ему было понятно и приятно.`
  ...
  function ensureVipAiModalStyles() {
    ...
    style.textContent = '.documents-vip-ai{position:fixed;inset:0;background:linear-gradient(180deg,rgba(226,232,240,.34),rgba(148,163,184,.28));backdrop-filter:blur(10px);z-index:4100;display:flex;align-items:stretch;justify-content:center;padding:4px}.documents-vip-ai__panel{width:100%;height:100%;max-height:none;overflow:auto;border-radius:20px;background:linear-gradient(145deg,rgba(255,255,255,.94),rgba(248,250,252,.9));border:1px solid rgba(255,255,255,.92);box-shadow:0 18px 44px rgba(15,23,42,.16)}.documents-vip-ai__head{padding:14px 16px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid rgba(226,232,240,.9);position:sticky;top:0;background:rgba(255,255,255,.78);backdrop-filter:blur(8px);z-index:2}.documents-vip-ai__title{font-size:18px;font-weight:800;color:#0f172a}.documents-vip-ai__sub{font-size:12px;color:#64748b;margin-top:3px}.documents-vip-ai__close{border:none;background:rgba(255,255,255,.95);width:34px;height:34px;border-radius:10px;color:#334155;font-size:20px}.documents-vip-ai__body{padding:14px;display:grid;gap:10px}.documents-vip-ai__meta{display:flex;flex-wrap:wrap;gap:8px}.documents-vip-ai__chip{padding:6px 10px;border-radius:999px;background:rgba(255,255,255,.84);border:1px solid rgba(203,213,225,.95);font-size:12px;color:#334155}.documents-vip-ai__block{border:1px solid rgba(203,213,225,.9);background:rgba(255,255,255,.78);border-radius:14px;padding:11px}.documents-vip-ai__label{font-size:12px;color:#64748b;margin-bottom:7px}.documents-vip-ai__files{display:grid;gap:6px;font-size:13px;color:#0f172a;max-height:20dvh;overflow:auto}.documents-vip-ai__chat{height:min(50dvh,520px);overflow:auto;display:flex;flex-direction:column;gap:8px}.documents-vip-ai__msg{padding:9px 10px;border-radius:12px;font-size:13px;line-height:1.45;white-space:pre-wrap}.documents-vip-ai__msg--user{align-self:flex-end;background:#dbeafe;color:#1e3a8a}.documents-vip-ai__msg--assistant{align-self:flex-start;background:#fff;color:#0f172a;border:1px solid rgba(203,213,225,.9)}.documents-vip-ai__composer{display:flex;gap:8px;align-items:center}.documents-vip-ai__select{flex:1;border:1px solid rgba(203,213,225,.95);border-radius:12px;min-height:44px;padding:0 12px;font-size:13px;background:#fff;color:#0f172a;-webkit-appearance:menulist;appearance:menulist}.documents-vip-ai__template{border:none;min-height:44px;padding:0 14px;border-radius:12px;background:rgba(219,234,254,.95);color:#1e3a8a;font-weight:700;white-space:nowrap}.documents-vip-template-preview{position:fixed;inset:0;z-index:4200;background:rgba(15,23,42,.42);backdrop-filter:blur(10px);display:flex;align-items:stretch;justify-content:center;padding:0}.documents-vip-template-preview__card{width:100%;height:100dvh;display:flex;flex-direction:column;border-radius:0;border:none;background:linear-gradient(160deg,rgba(255,255,255,.97),rgba(241,245,249,.94));overflow:hidden}.documents-vip-template-preview__head{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:10px 12px;border-bottom:1px solid rgba(203,213,225,.8);background:rgba(255,255,255,.86);backdrop-filter:blur(8px)}.documents-vip-template-preview__title{font-size:15px;font-weight:800;color:#0f172a}.documents-vip-template-preview__close{border:1px solid rgba(203,213,225,.9);background:rgba(255,255,255,.92);border-radius:10px;width:36px;height:36px;color:#0f172a}.documents-vip-template-preview__body{flex:1;overflow:auto;padding:12px;background:#e2e8f0}.documents-vip-template-preview__loading{height:100%;min-height:180px;display:flex;align-items:center;justify-content:center;text-align:center;padding:18px;color:#334155;font-size:14px}.documents-vip-template-preview__body .docx-wrapper{background:transparent;padding:0;display:flex;justify-content:center}.documents-vip-template-preview__body .docx{margin:8px auto 24px;box-shadow:0 18px 40px rgba(15,23,42,.24)}.documents-vip-template-preview__body .docx,.documents-vip-template-preview__body .docx *{font-family:\"Times New Roman\",Times,serif!important;letter-spacing:normal!important}.documents-vip-ai__error{color:#b91c1c}@media (max-width:768px){.documents-vip-ai{padding:0}.documents-vip-ai__panel{border-radius:0}.documents-vip-ai__body{padding:12px}.documents-vip-ai__chat{height:52dvh}.documents-vip-ai__composer{flex-direction:column}.documents-vip-ai__template{width:100%}.documents-vip-template-preview__body{padding:8px}}';
  ...
  function ensureDocxPreviewLoaded() {
    ...
    var loadZip = window.JSZip
      ? Promise.resolve()
      : loadBriefScript('https://cdn.jsdelivr.net/npm/jszip@3.10.1/dist/jszip.min.js', function() { return Boolean(window.JSZip); });
    return loadZip
      .then(function() {
        return loadBriefScript('https://cdn.jsdelivr.net/npm/docx-preview@0.3.6/dist/docx-preview.min.js', function() {
          return Boolean(window.docx && typeof window.docx.renderAsync === 'function');
        });
      })
      .then(function() { return window.docx; });
  }

  function buildTemplateDocxPreviewBuffer() {
    var templateUrl = '';
    try {
      templateUrl = new URL(TEMPLATE_DOC_PATH, window.location.origin).toString();
    } catch (_) {
      templateUrl = TEMPLATE_DOC_PATH;
    }
    return fetch(templateUrl, { credentials: 'same-origin', cache: 'no-store' })
  ...
  function openTemplatePreviewModal() {
    ...
    overlay.className = 'documents-vip-template-preview';
    overlay.innerHTML = '<div class="documents-vip-template-preview__card"><div class="documents-vip-template-preview__head"><div class="documents-vip-template-preview__title">Просмотр template.docx</div><button type="button" class="documents-vip-template-preview__close" data-close aria-label="Закрыть">×</button></div><div class="documents-vip-template-preview__body" data-preview-body><div class="documents-vip-template-preview__loading">Подготавливаем точный рендер документа…</div></div></div>';
    overlay.addEventListener('click', function(event) {
      if (event.target === overlay) {
        overlay.remove();
      }
    });
    var closeButton = overlay.querySelector('[data-close]');
    if (closeButton) {
    ...
    return {
      root: overlay,
      body: overlay.querySelector('[data-preview-body]')
    };
  }
  ...
  function openDocumentsVipAiPaidModal(config) {
    ...
    var templateButton = panel.querySelector('[data-template-btn]');
    ...
    if (templateButton) {
      templateButton.addEventListener('click', function() {
        pushChat('assistant', 'Открываю предпросмотр шаблона...');
        var preview = openTemplatePreviewModal();
        var previewBody = preview && preview.body;
        if (!previewBody) {
          pushChat('assistant', 'Не удалось открыть окно предпросмотра.');
          return;
        }
        Promise.resolve()
          .then(function() { return ensureDocxPreviewLoaded(); })
          .then(function(docxLib) {
            return buildTemplateDocxPreviewBuffer().then(function(buffer) {
              previewBody.innerHTML = '';
              return docxLib.renderAsync(buffer, previewBody, null, {
                className: 'docx',
                inWrapper: true,
                ignoreWidth: false,
                ignoreHeight: false,
          ...
          .catch(function(error) {
            pushChat('assistant', 'Не удалось показать шаблон: ' + (error && error.message ? error.message : 'неизвестная ошибка'));
          });
      });
    }
  }
