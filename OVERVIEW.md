...
- Для `[ГОД]`:
  - если поле не заполнено, backend автоматически подставляет текущий год.
- Для `[ОТВЕТ ИИ]`:
...
- Автономный OCR-воркер в проект не входит; фоновое распознавание вложений не запускается.
- Новые входящие вложения, загруженные при создании или обновлении задачи, сохраняются в режиме `manual_only`: они не получают OCR-статусы, не попадают в очередь и не подбираются последующим backfill. Фоновая S3-синхронизация выполняется отдельно от OCR.
- В ручном админском тесте PDF читается локальным PDF.js: текстовый слой извлекается без `pdftotext`, а сканы и изображения по явному нажатию распознаются в браузере через Tesseract.js. Для DOCX/XLSX нужны PHP-расширения `zip` и `dom`, TXT читается средствами PHP.
- В админке оставлено только модальное окно «OCR-тест» для ручной проверки PDF, изображений, DOCX, XLSX и TXT; мониторинг очереди и worker-статусы из интерфейса удалены. Tesseract.js загружается только после нажатия «Распознать текст» и не участвует в автоматической очереди или воркере. Отдельная страница `ocr-test.html` остаётся изолированным ручным тестом того же браузерного механизма.
- Поиск в Telegram Mini App строит актуальный снимок задач из серверных реестров и добавляет OCR-текст онлайн; клиент не передаёт и не может подменить OCR-содержимое.
- Ручной тест в модальном окне не использует пакетную очередь. PDF и изображения обрабатываются в браузере, DOCX/XLSX/TXT отправляются одним HTTP-запросом, а общий лимит ручной операции составляет 60 секунд.
