...
- Загрузка задачи только добавляет вложения в долговечную очередь; Tesseract не запускается внутри PHP-FPM-запроса.
- Очередь последовательно обрабатывает `ocr-worker.php`. Встроенная блокировка не допускает одновременный запуск двух воркеров.
- Для автоматической обработки добавьте в crontab пользователя сайта запуск раз в минуту:

  ```cron
  * * * * * /usr/bin/php /var/www/www-root/data/www/bimmax.pro/ocr-worker.php --max-jobs=3 --max-runtime=50 >/dev/null 2>&1
  ```

- Состояние очереди, повторы, зависшие задания и предохранитель нагрузки доступны в модальном окне «OCR» в админке.
- Порог нагрузки можно настроить переменной `BIMMAX_DOCS_OCR_MAX_LOAD`; без неё он рассчитывается по числу процессорных ядер и ограничивается безопасным максимумом.
