...
async function requestTelegramBriefAiByAttachment(source, aiMode = 'free') {
  ...
  const fileName = normalizeValue(source && source.label) || 'Файл';
  if (!fileUrl) throw new Error('Не найден URL файла.');
  const extractedText = await requestTelegramOcrByUrl(fileUrl);
  return requestTelegramBriefAi(fileName, extractedText, aiMode);
}

async function requestTelegramBriefAiDirectWithAttachment(source) {
  const fileUrl = normalizeValue(source && source.url);
  const fileName = normalizeValue(source && source.label) || 'Файл';
  if (!fileUrl) throw new Error('Не найден URL файла для VIP режима.');
  const extractedText = await requestTelegramOcrByUrl(fileUrl);
  return requestTelegramBriefAi(fileName, extractedText, 'paid');
}
