...
function createResponseUploadControls(task, entry, setStatus) {
  ...
  const openVipTelegramModal = async () => {
    ...
    const buildPaidAiFileCandidates = (file) => {
      const directSources = [
        normalizeValue(file && file.resolvedUrl),
        normalizeValue(file && file.url),
        normalizeValue(file && file.previewUrl),
      ].filter(Boolean);
      const resolvedSources = directSources
        .map((value) => resolveDocumentUrl(value))
        .filter(Boolean)
        .map((value) => appendCacheBuster(toAbsoluteUrl(value)));
      const primary = resolveFileFetchUrl(file);
      const all = [primary, ...directSources, ...resolvedSources].filter(Boolean);
    ...
    if (filesGrid) {
      filesGrid.innerHTML = files.length
        ? files.map((file, index) => {
          ...
          return `<label class="tg-paid-chat__file"><input type="checkbox" data-file-key="${fileKey}"><span>${escapeHtml(fileName)}</span></label>`;
        }).join('')
    ...
    filesGrid?.addEventListener('change', () => {
      const checkedCount = filesGrid.querySelectorAll('input[type="checkbox"][data-file-key]:checked').length;
      statusNode.textContent = checkedCount
        ? `Выбрано файлов: ${checkedCount}.`
    ...
    overlay.querySelector('[data-send]')?.addEventListener('click', async (event) => {
      ...
      try {
        const selectedInputs = Array.from(
          filesGrid ? filesGrid.querySelectorAll('input[type="checkbox"][data-file-key]:checked') : [],
        );
        const filesToSend = [];
        for (const inputNodeItem of selectedInputs) {
          const key = normalizeValue(inputNodeItem && inputNodeItem.dataset && inputNodeItem.dataset.fileKey);
          if (key === '') {
            continue;
          }
          const file = files[Number(key)];
          const candidateUrls = buildPaidAiFileCandidates(file);
          if (!candidateUrls.length) {
            throw new Error('У выбранного файла нет доступной ссылки для загрузки.');
          }
          let fileBlob = null;
          for (const candidateUrl of candidateUrls) {
            ...
            try {
              // eslint-disable-next-line no-await-in-loop
              const fileResponse = await fetch(candidateUrl, { credentials: 'include' });
              if (!fileResponse.ok) {
                continue;
              }
              // eslint-disable-next-line no-await-in-loop
          ...
          if (!fileBlob) {
            throw new Error('Не удалось скачать один из выбранных файлов. Попробуйте другой файл.');
          }
