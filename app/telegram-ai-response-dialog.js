...
function openAiResponseDialog(context = {}) {
  ...
  const getPreparedContextForAi = () => {
    const source = Array.isArray(context.extractedTexts) ? context.extractedTexts : [];
    const normalized = [];
    let total = 0;
    for (let i = 0; i < source.length; i += 1) {
      if (normalized.length >= MAX_AUTO_CONTEXT_FILES || total >= MAX_AUTO_CONTEXT_TEXT_CHARS) break;
      const item = source[i] || {};
      const text = String(item.text || '').trim();
      if (!text) continue;
      const remain = Math.max(0, MAX_AUTO_CONTEXT_TEXT_CHARS - total);
      const clipped = text.slice(0, remain);
      if (!clipped) continue;
      normalized.push({
        name: item.name || `Файл ${i + 1}`,
        type: item.type || 'text/plain',
        text: clipped,
      });
      total += clipped.length;
    }
    return { ...context, extractedTexts: normalized };
  };
  ...
  const requestAssistantWithFallback = async (prompt, history) => {
    const preparedContext = getPreparedContextForAi();
    try {
      return await requestAssistantReply(prompt, { ...preparedContext, model: state.model }, history);
    } catch (error) {
      if (error && error.code === 'MODEL_NOT_ALLOWED' && Array.isArray(error.availableModels) && error.availableModels.length) {
        ...
        return requestAssistantReply(prompt, { ...preparedContext, model: state.model }, history);
      }
      if (preparedContext.extractedTexts && preparedContext.extractedTexts.length) {
        return requestAssistantReply(prompt, { ...preparedContext, extractedTexts: [], model: state.model }, history);
      }
      throw error;
    }
  };
